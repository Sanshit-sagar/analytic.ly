              ...
              {Object.entries(tooltipData.data).map((datum, i) => {
                return (          
                  <Flex css={{ fd: 'row', jc: 'space-between', ai: 'flex-start'}}> 
                    <Text size='1' css={{ textDecoration: 'underline', textDecorationColor: '$green' }}> {SANITIZED_KEYS[datum[0]]} </Text>
...
const HeatedGeo = () => {
  ...
  let data = Object.entries(geodata)
  data.map((d, i) => {
    if(data[i] && data[i][0] && data[i][1]) {
      markers[`${data[i][1].hash}`] = {
        ...
        postalCode: data[i][1].postalCode || '',
        metroCode: data[i][1].metroCode || '',
