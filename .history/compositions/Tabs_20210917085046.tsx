...
const TabsContentWrapper = ({ index, value, size, children }: ITabsContentWrapperProps) => (isSmall(size) 
    ? <TabsContentSmall key={index} value={value}> {children} </ 
    : <TabsContentLarge key={index} value={value}/>
);
           ...
           <TabsHorizontalList items={items} />
            ...
            {items.map((item: ITabItem, index: number) => (
                <TabsContentWrapper 
                    ...
                    size={size} 
                    value={item.value}
                    chidlren={item.content || <EmptyTab />}
                />
            ))}
        </Tabs>
    )
