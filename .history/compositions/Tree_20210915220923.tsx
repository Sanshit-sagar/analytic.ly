...
interface IItem {
    name: string; 
    items: { id: string; url: string; percentage: number; }[];
}

export const CustomTree = ({ slug = 'default_slug_500' }: ICustomTreeProps) => {
    let tree = useTreeData({
        initialItems: [{ 
            name: 'test0',
            items: { id: '0', url: '', percentage: 50 },
        }, { 
            name: 'test1', 
            items: { id: '1', url: '', percentage: 50 }, 
        }],
        initialSelectedKeys: [],
        getKey: (item: IItem) => item.name,
