...
const TabsContentWrapper = ({ key, value, size, children }: ITabsContentWrapperProps) => {

    return (size==='small') ? <TabsContentSmall key={index} value={value}/> : <TabsContentLarge key={index} value={value}/>
}
    ...
    return (
        ...
        >
           <TabsHorizontalList items={items} />
            ...
            {items.map((item: ITabItem, index: number) => (
                <TabsContentWrapper key={index} size={size} value={item.value}>
                    {item.content || <EmptyTab />}
                </TabsContentSmall>
            ))}
        </Tabs>
