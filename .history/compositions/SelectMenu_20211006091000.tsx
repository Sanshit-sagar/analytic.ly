        ...
        >
            <SelectTrigger>
                <Tooltip content={group}>
                    <Text css={{  width: '100%', display: 'inline-flex', jc: 'space-between', ai: 'center', gap: '$2', }}>  
                        <> {
                                typeof selectedTextValue === 'string' 
                            ?   <Text css={{ fontSize: '11px' }}> {sanitize(selectedTextValue)} </Text>
                            :   selectedTextValue
                        } </> 
                        <TriangleDownIcon />  
                    </Text>
...
function SelectMenu(props: ISelectMenuProps) {
    ...
    return (
        <ControlGroup>
            <Label>{props.group.toLowerCase() || ''}</Label>
            <Box css={{ padding: '0' }}>
                <SelectMenuLoc {...props} />
            </Box>
        </ControlGroup>
