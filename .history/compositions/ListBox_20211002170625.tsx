...
const OptionActions = ({ }) => {
    return (
        <Text css={{  color: isHovered ? 'green' : '$funkyText',  mt: '1px' }}>
         ...
         <Text css={{  color: isHovered ? 'red' : '$funkyText',  mt: '1px' }}>
         <TrashIcon /> 
     </Text>
    )
} 
  ...
  let ref = React.useRef<HTMLLIElement>(null)
    ...
    return (
        ...
        >
            ...
            >
                 <Text size='1'> {item.rendered} </Text> 
            </OptionContext.Provider>

            {isSelected && (
                <Flex css={{ width: '100%', fd: 'row', jc: 'flex-end', ai: 'center', gap: '$1' }}>
                    <ToggleSelectIcon isSelected={isSelected} isHovered={isHovered} isPressed={isPressed} isDisabled={isDisabled} isFocussed={isFocussed} />
