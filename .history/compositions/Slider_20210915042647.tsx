...
import { useSlider, useSliderThumb, AriaSliderProps } from '@react-aria/slider'
...
interface INumberFieldProps {
    thumbCount: number; 
    onThumbCountChange: (value: number) => void;  
    minThumbs: number;
    maxThumbs: number;
    formatOptions: Intl.NumberFormatOptions; 
    groupName: string;
    sliderProps: AriaSliderProps; 
};

const ThumbCountController = ({ thumbCount, onThumbCountChange, minThumbs = 2, maxThumbs = 10, formatOptions = { style: 'percent' },
    groupName}: INumberFieldProps) => {

    return (
        <NumberField
            label={groupName}
            value={thumbCount}
            onChange={onThumbCountChange}
            minValue={minThumbs}
            maxValue={maxThumbs}
        />
    )
...
export function RangeSlider(props: any, numberFieldProps: ) {
    ...
    return (
        <BorderlessControlGroup>
            <Flex css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'flex-start', gap: '$2'}}>
                ...
                <StyledOutputText outputProps={outputProps} children={`ALTERNATE URLs: ${values?.length}`} /> 
                { withNumberField && thumbCount && <ThumbCountController props={...numberFieldProps} /> }
            </Flex>
...
export const SliderWithAdjustableThumbSet = ({ 
    thumbCount, 
    onThumbCountChange,
    minThumbs = 2,
    maxThumbs = 10, 
    ...
    let { groupProps, trackProps, labelProps, outputProps } = useSlider(sliderProps, state, trackRef);
    let values: number[] = [...state?.values] || []
    let realValues = sliderProps.values;
