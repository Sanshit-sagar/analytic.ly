...
export const CustomTree = ({ treeNodes }: { treeNodes: ITreeNode[] }) => {
    ...
    return (
        <TreeContainer>
            {treeNodes.map((node:ITreeNode, index: number) => {
                if(!node) return null; 
                ...
                if(!node.children?.length) {
                    return (
                        <Tree 
                            key={index}
                            name={node.name}
                            defaultOpen={node.defaultOpen, style={node.style}}
                    )
                }
                
                if(node.children?.length) {
                    let childNodes: ITreeNode = node.children.map((childId) => {
                        childNodes = childNodes?.length 
                            ? [...childNodes, treeNodes[childId]] 
                            : [treeNodes[childId]];
                    });
                }
