...
export const CustomTree = ({ treeNodes }: { treeNodes: ITreeNode[] }) => {

    return (
        <>
            {treeNodes.map((node:ITreeNode) => {
                if(!node) return null; 

                if(!node.children?.length || node.isLeaf || node.content) {
                    return (
                        <Tree 
                            key={index}
                            name={node.name}
                ...
                let childNodes: ITreeNode = node.children.map((childId: string) => {
                    childNodes = childNodes?.length 
                        ? [...childNodes, treeNodes[parseInt(childId)]] 
                        : [treeNodes[parseInt(childId)]];
                });
...
export const CustomTreeWrapper = () => {
    ...
    return (
        <CustomTree treeNodes={treeNodes} />
    )
}