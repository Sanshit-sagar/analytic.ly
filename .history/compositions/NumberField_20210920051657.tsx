...
const Message = ({ props, message }: { props: any; message: string; validation: ValidationStateType }) => (
    <MessageText {...props}>
       <Text css={{ color: status===ValidationStateEnum.INVALID ? 'red' : status==='description' ? '$funkyText' : '$text' }}>
            {message}
        </Text> 
    </MessageText> 
); 
