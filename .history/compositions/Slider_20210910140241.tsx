...
interface IThumbsGroupProps {
    sliderProps: IAriaSliderProps;
    state: SliderState;
    trackRef: React.RefObject<HTMLElement>;
    numThumbs: number; 
}

const ThumbsGroup = ({ props, state, trackRef, numThumbs }: IThumbsGroupProps) => {
    const sliderProps: IAriaSliderProps = props
    const stepSize: number = sliderProps?.step || 0.01
    let min: number = !sliderProps?.minValue ? 0 : Math.max(0, sliderProps?.minValue)
    let max: number = !sliderProps?.maxValue ? 100 : Math.min(100, sliderProps?.maxValue)
    const partitionSize: number = (max-min)/(state.step*100 * thumbsCount)
