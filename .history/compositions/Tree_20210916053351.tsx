...
const SingleColumnTree = ({ name, items, setItems, ItemType, value, removeItem }: { name: string; value: string; removeItem: () => void; }) => {

    const [abTestConfig, setAbTestConfig] = useAtom(abTestConfigAtom)

    const updateUrlAtIndex = (event: React.ChangeEvent<HTMLInputElement>) => (
        setAbTestConfig((oldValue: ItemType) => ({ 
            ...oldValue, 
