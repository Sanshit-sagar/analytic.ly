...
export const Tree = memo<TreePropsType>(({ children, name, style, defaultOpen = false }) => {
    ...
    return (
        <TreeFrame>
            ...
            <TreeNodeContent style={{ opacity, height: isOpen && previous === isOpen ? 'auto' : height }}>
                <a.div 
                    ref={ref} 
                    style={{ y }} 
                    children={children} 
...
const IconButtonWithTooltip = ({ label, onClick, icon }: { label: string; onClick: () => void; icon: React.ReactNode | Element}) => {
