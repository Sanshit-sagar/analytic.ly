...
export const Meter = ({ label, showValueLabel = !!label, value = 10, minValue = 0, maxValue = 100 }: AriaMeterProps) => {
    ...
    const meterWidth = useAtomValue(meterPercentageAtom)
    const meterColor = useAtomValue(meterColorAtom)

    const [meterValue, setMeterValues] = useUpdateAtom(meterValueAtom)

    const incr = () => setMeterValue(meterValue + 10 <= 100 ? meterValues + 10)
    const decr = () => setMeterValue(meterValue - 10 >= 0 ? ? meterValues - 10);
    ...
    return (
        <MeterContainer {...meterProps}>
            <MeterInfoBar>
                ...
                <button onClick={increasePct}> click me </button>
            </MeterInfoBar>
