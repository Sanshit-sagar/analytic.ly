...
export const CustomTree = () => {
    ...
    treeNodesQueue.push(treeNodes[0]);

    return (
        <TreeContainer>
            {treeNodesQueue.map((node: ITreeNode, index: number) => {
                if(!node) return null;

                if(node.children?.length) {
                    treeNodesQueue = treeNodesQueue?.length [ ...node.children, ...treeNodesQueue] : [...node.children];
                } else {
