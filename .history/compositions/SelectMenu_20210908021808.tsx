...
const isSelected = (index: number, selectedIndex: number): boolean => (
    index===selectedIndex
);

const getHashFromValue = (group: string, a: string, b?: string):string => (
    `${group}-item-${a}-value-${b || ''}`
const getHashFromIndex = (group: string, i: number) => `${group}-item-index-${i}`
const hash = (controlGroupName: string, a?: string, b?: string, i?: number): string => {
     let cgn = controlGroupName || `Group-${Math.random()}`
     return a && b ? getHashFromValue(cgn, a, b) : i ? getHashFromIndex(cgn, i) : `${Math.random()}`;
};


const SelectionIndicator = ({index, selectedIndex}: ISelectionIndicator) => {
