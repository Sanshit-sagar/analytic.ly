...
const FilteredItemCount = ({ id, items, inputValue, commit }: { 
    ...
    let max = Math.max(0, -100)
    useEffect(() => max=Math.max(max, items.length)
 []); 
    <OutputGroup>
        {[...items]?.length ? 
            <InfoMessage> 
               Showing {[...items]?.length ?? 0}/{} {[...items].length<=1 ? 'result' : 'results'} 
            </InfoMessage>
        : inputValue?.length ? 
                ...
                <Flex css={{ width: '100%', fd: 'column', jc: 'flex-start', ai: 'flex-end', mt: 0, gap: '$1' }}>
                    ...
                    >
                        <Text size='1' css={{ color: '$funkyText'}}> save {inputValue} ? </Text>
                    </Button>
                    <InfoMessage> 0 matches </InfoMessage>
                </Flex>
        : null 
        }
    </OutputGroup>
