...
import { abTestGroupDiversionPercentAtom } from '../components/SubMenus/AbTestingTab/index'
...
function Thumb(props: any) {
    ...
    return (
        <Flex css={{ width: '100%', fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$3' }}>
            <Text css={{ color: '$accentPressed' }}> {`${Math.round(state.getThumbPercent(index) * 100)}%`} </Text>
            <ThumbContainer 
...
export function RangeSlider(props: any) {
    const setAbPercentagesAtom = useUpdateAtom(abTestGroupDiversionPercentAtom)
    ...
    useEffect(() => {
        setAbPercentagesAtom([...state?.values] || [])
    }, [...state?.values]);
