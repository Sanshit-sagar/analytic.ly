...
const Save = ({ props }:{ props: ComboBoxProps<any> }) => {
    ...
    return isValidKey(props.items, props.inputValue || '') ? <Button onClick={handleSave}> Save? </Button> : null;
}
...
const GroupInfo = ({ label }: Intr & { label: string }) => (
...
export const ComboBox = (props: ComboBoxProps<any>) => {
    ...
    let inputRef: MutableRefObject<HTMLInputElement | HTMLTextAreaElement | null> = useRef(null)
    let listBoxRef: MutableRefObject<HTMLElement | null> = useRef(null)
    let popoverRef:  MutableRefObject<HTMLDivElement | null> = useRef(null)
    let buttonRef: MutableRefObject<HTMLElement | null> = useRef(null)
