...
function isValidKey(items: ComboBoxProps<any>) {
    return (!props.items || !props.items?.done || props.items?.done===0) && props?.inputValue?.length!==0
}
...
export const ComboBox = (props: ComboBoxProps<any>) => {
    ...
    let inputRef: RefObject<HTMLInputElement | HTMLTextAreaElement | null> = useRef(null)
    let listBoxRef: RefObject<HTMLElement | null> = useRef(null)
    let popoverRef:  RefObject<HTMLDivElement | null> = useRef(null)

    let buttonRef: RefObject<HTMLElement | null> = useRef(null)
