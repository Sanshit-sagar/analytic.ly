...
import { abTestPercentagesAtom, abTestNumGroupsAtom  } from '../components/SubMenus/AbTestingTab/index'
...
export function RangeSlider(props: any) {
    ...
    let groupCount = useAtomValue(abTestNumGroupsAtom)
    let setAbPercentages = useUpdateAtom(abTestPercentagesAtom)
    ...
    useEffect(() => {
        if(groupCount >= 2 && groupCount < state?.values.length){
            let tempValues: number[] = [...Array(groupCount)].map((e: number, i: number) => state.values[i])
