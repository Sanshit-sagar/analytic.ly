...
export const ComboBox: React.FC<ComboBoxComponentProps> = (props: ComboBoxComponentProps) => {
    ...
    const { children, items, item }: ComboBoxStateProps = useComboBoxState({ ...props, defaultFilter: contains })
    const ({title, children, isOpen, onClose, style, ...otherProps}, ref) => {

        const { overlayProps } = useOverlay({ onClose: true, isOpen, :false, isDismissable: true })
    ...
    let { 
        ...
        listBoxProps, 
        labelProps 
    } = useComboBox({ ...props, inputRef, buttonRef, listBoxRef, popoverRef }, );
    ...
    return (
        <ControlGroup>
            ...
            <FlexInlineBlock>
                <InputGroup> 
                    <StyledInput ref={inputRef} {...inputProps} isFocused={state.isFocused} /> 
