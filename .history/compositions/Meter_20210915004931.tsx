...
const isCondition1MetAtom = ((get) => get(passwordAtom).length >= 5)
const isCondition2MetAtom = ((get) => get(passwordAtom).charAt(3) === 'a' && get(passwordAtom).charAt(4) === '1')

export const Password = () => {
    const [meterPct, setMeterPct] = useAtom(meterPercentageAtom)

    const incr = (amt: number) => setMeterPct(Math.min(100, amt + meterPct))
    const decr = (amt: number) => setMeterPct(Math.max(0, amt - meterPct))
