...
function noMatchesForUserInput(props: ComboBoxProps) {
    (!props.items || !props.items?.length || props.items?.length===0) && props?.inputValue?.length!==0
}
               ...
               <Flex css={{ fd: 'row', jc: 'space-between', ai: 'stretch' }}> 
                ...
                {!state.isOpen &&
                    <Box css={{ pt: '$2', border: 'none', bc: 'transparent',  fd: 'row', jc: 'space-between', ai: 'flex-start', gap: '$1' }}>
                         <>{noMatchesForUserInput(props) ? 
                             <Button onClick={() => alert(`Saving new parameter`)}> 
                                 Save? 
                             </Button>
                             : null
                         }</>
