...
const ThumbsGroup = ({ sliderProps, state, trackRef }: IThumbsGroupProps) => {
    const sliderProps: IAriaSliderProps = {...sliderProps}
    const thumbsCount = 3;
    const stepSize = 0.01
    let min: number = !state?.minValue ? 0 : Math.max(0, state?.minValue)
    let max: number = !state?.maxValue ? 100 : Math.min(100, state?.maxValue)
