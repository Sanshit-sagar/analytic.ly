...
type KeyedArrayType<T> =  { 
    [key: string]: T;
}[] 

const toKeyedArray<T extends object>: (obj: KeyedArrayType<T>) => {
    const methods = {
        map(target: KeyedArrayType<T>) {
            return ((callback) => {
                return Object.keys(target).map((key: string) => {
                    return callback(target[key], key, target)
                })
            })
        },
        reduce(target: KeyedArrayType<T>) {
            return (callback, accumulator) => {
                Object.keys(target).reduce(
                    (acc, key: string) => {
                        return callback(acc, target[key], key, target), accumulator;
        ...
        forEach(target) {
            return ((callback) => {
                return Object.keys(target).forEach((key: string) => {
                    return callback(target[key], key, target)
                })
            })
        },
