...
const addDaysToDate = (date: number, additionalDayCount: number) => {
    const d = new Date(date)
    d.setDate(d.getDate() + additionalDayCount)
    return d.toISOString().split('T')[0]
}

const addMinutesToDate = (date: number, additionalMinutes) => {
    const d = new Date(date);
    d.setTime(d.getTime() + additionalMinutes * 60000);
    return d.toISOString().split('.')[0].replace('T',' ');
};

const addWeekDays = (startDate: Date, additionalDaysCount: number) => {
    return Array.from({ length: additionalDaysCount }).reduce(dateWithFocus: Date => {
        
        dateWithFocus = new Date(dateWithFocus.setDate(dateWithFocus.getDate() + 1))
