...
export function getMinutelySeries(startTimestamp: number, endTimestamp: number): TimeSeries {
    const duration: number = endTimestamp - startTimestamp
    ...
    const minuteAtI = (i: number) => {
        return ((startMinute + i + 1)%60) < 10 ? `0${(startMinute + i + 1)%60}` : (((startMinute + i + 1)%60));
    }

    const hourAtI = (i: number) => {
        return startHour + ((startMinute + i + 1)/60)%24;
    }

    const flooredStart = setTimeForDate(new Date(startTimestamp), startHour, 0, 0).getTime(); 
    const timeseries: TimePeriod[] = new Array(numMinutes).fill(null).map((_, i) => ({
        x: `${(startMinute + i)%60}`,
        y: 0,
        minute: `${(startMinute + i)%60}`,
        hour: (startHour + i + 1)%24,
        date: Math.floor(startDate + ((startHour + i + 1)/24)), 
        month: Math.floor(startMonth + (startDate + ((startHour + i + 1)/24)) % daysInMonth(startMonth + (startDate + ((startHour + i + 1)/24))))
    })); 

    return { timeseries, duration, numPeriods: numMinutes, start: new Date(startTimestamp).getTime() }
}
