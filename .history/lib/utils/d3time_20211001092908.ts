...
export function getValueInMillis(text: string): number | undefined {
    try {
        ...
        if(textParts && textParts.length > 1) {
            let multiplier = parseInt(textParts[0])
            if(typeof multiplier === 'number' && multiplier>0) {
                let duration = getValueOfWordInMillis(textParts[1] || '')
                return duration ? multiplier * duration : undefined
            }
        }
        return getValueOfWordInMillis(text)
    } catch(error) {
        return undefined 
    }
}

export function getValueOfWordInMillis(text: string): number | undefined {
    try {
...
export function getLabelsFromBounds(bounds: number[], numTicks: number, outputFmt: string = 'dayhourmin') {
    ...
    for(i=bounds[0]; i<=bounds[1] && ticks.length<=numTicks; i+=intervalLen) {
...
export function formatClickDate(x: number, minTimestamp: number, interval: string): { timestamp: number, fmtTimestamp: string } {
