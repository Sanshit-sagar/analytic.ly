...
export function normalizeOptions(rawOptions: IRawOptions) {
    const hasOmittedWords = rawOptions.wordsToMantainCaseFor?.length > 0 ? true : false
    let normalizedOptions: IRawOptions = {
        ...rawOptions,
        titleCase: rawOptions.titleCase ? hasOmittedWords ? [...rawOptions.wordsToMantainCaseFor] : false : hasOmittedWords ? [...rawOptions.wordsToMantainCaseFor] : true,
