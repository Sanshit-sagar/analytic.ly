...
export function generateTimeseries(start, end) {

    let clickstream = await getDoubleEndedClickstream(start, end, true); 
    let timeseries = await getHourlySeries(start, end).timeseries;
    let freqs = createFrequencyMap(timeseries, clickstream); 
    let timeseries = populateTimeseriesWithFreqs(freqs, timeseries)
    return timeseries; 
}