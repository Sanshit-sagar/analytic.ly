...
function getPeriodStats(currentTs: number, increment: number, startTs: number): IProgress {
...
export function aggregateClicksForTimeseries(timeseries: TimeSeries, clickstream: any): TimeSeries[] {
    ...
    timeseries.timeseries.map((period, index) => {
        ...
        let periodClickCount: Number | undefined | 0 = aggregatedClicks.has(periodStart) ? aggregatedClicks.get(periodStart) : 0; 
