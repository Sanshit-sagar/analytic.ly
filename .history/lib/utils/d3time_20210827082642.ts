...
export function getLabelsFromBounds(bounds: number[], numTicks: number, outputFmt?: string) {
    let i;
    let ticks: string[] = [];
    const timespan = bounds[1] - bounds[0];
    const intervalLen = numTicks===0 ? timespan - 1 : timespan/numTicks;

    for(i=bounds[i]; i<=bounds[1]; i+=intervalLen) {
        ticks.push(format(bounds[i]), )
    }
}
