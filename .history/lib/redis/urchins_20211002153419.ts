...
export async function deleteUrchinFromCategoryForUser(email: string, key: string, value: string): Promise<any> {
    if(!email || !key || !value || !isValidUtm(key)) {
        Promise.reject('INVALID PROPS')
    }

    try {
        const data = await redis.srem(`urchins.${key}.user.${email}`, )
        Promise.resolve(data)
    } catch (error) {
        Promise.reject(error)
    }
