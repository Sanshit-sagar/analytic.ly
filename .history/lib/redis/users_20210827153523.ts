...
export async function getStatisticsForUser(email: string): Promise<{ index: number; name: string; result: any }[]> {
    // validate email here --> email
    let userStatistics: { index: number; name: string; result: any }[] = []; 
    let statisticNames: string[] = ['Total Page Views', 'Unique Visitors']
