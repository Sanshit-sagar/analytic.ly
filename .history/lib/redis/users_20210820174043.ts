...
export async function getUniqueVisitorsForUser(email: string, limit?: number): Promise<any>  {
    ...
    const totalViews: any = {}; 
    const userClickEventDetails: any = []
    ...
    details.map((detail: any, _: number) => {
        ...
        if(!firstSeenAt[clickEvent.slug]) {
            ...
            totalViews[clickEvent.slug] = 1; 
            clickDetails = [...Object.entries(clickEvent)]
        } else {
            totalViews[clickEvent.slug] += 1; 
            clickDetails = [...Object.entries(clickEvent)]
        }
        userClickEventDetails.push(clickDetails);
    });
    ...
    return {
        ...
        firsts: firstSeenAt,
        totals: totalViews,
        eventlog: userClickEventDetails
    }; 
}
