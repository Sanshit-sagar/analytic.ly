...
export const paginateClickstream = async (email: string | string[], prevIndex: number | undefined, pageSize: number | undefined): Promise<any> => {
    

    let query = email?.length ? `clickstream.user.${email}` : `clickstream`
    let startIndex = prevIndex || DEFAULT_START_INDEX
    let endIndex = startIndex + (pageSize || DEFUALT_PAGE_SIZE)

    let page: any[] = page = await redis.lrange(query, startIndex, endIndex);
    return { query, startIndex, endIndex, formatClickstream(page) };
}
