...
function isValidUtm(utm: string): boolean {
...
export async function addUrchinPairForUser(email: string, urchinKey: string, urchinValue: string): Promise<any> {
    if(!email || !urchinKey || !urchinValue || !isValidUtm(urchinKey)) Promise.reject('INVALID PROPS');


    try {
        const result = await redis.sadd('urchins')
    ...
    try {
        const multiPromise = await redis.multi()
            ...
            .exec((error: any, result: any) => {
                ...
                else result.map((res: any[], i: number) => urchins[utms[i]] = res);
            });
