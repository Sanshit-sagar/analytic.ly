...
const utm: UrchinKey[] = ['medium', 'source', 'term', 'campaign', 'content']
...
function isValidUtm(utm: string): boolean {
    return (utmFields.filter((validUtm) => utm===validUtm) || []).length === 1
}
    ...
    try {
        const multiPromise = await redis.multi()
            ...
            .exec((error: any, result: any) => {
                ...
                else result.map((res: any[], i: number) => urchins[utmFields[i]] = res);
            });
