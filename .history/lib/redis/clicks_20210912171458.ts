...
export const paginateClickstream = async (email: string | string[], cursor: number, pageSize: number | undefined): Promise<any> => {
    ...
    return { query: `${query},${start},${end}`, start, clickstream };
}
...
export const getClickstream = async (email: string | string[]): Promise<any[] | null> => {
    ...
    let clickstream: any[]  = await redis.lrange(query, 0, -1)
...
export const getClickFromField = async (fieldName: string | string[], fieldValue: string | string[]): Promise<any> => {
    if(fieldName?.length && fieldValue?.length) {
        return fieldName==='cfRay' ? getClickByCfRay(fieldValue) : null;
    }
    return null; 
}
