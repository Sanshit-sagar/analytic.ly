...
export async function getClickstreamOnDate(date: Date, isAsc: boolean): Promise<any[] | string> {
    ...
    return `Getting clicks on date ${new Date(timestampStart).toLocaleString()} to ${new Date(timestampEnd).toLocaleString()}`
