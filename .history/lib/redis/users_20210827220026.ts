...
export async function getSummaryForUser(email: string): Promise<any> {
    ...
    // get clickcount to find no. of slugs for the user, use it to compute slug sstats 

    const userSummary: any = { start: new Date(1970,1,1), end: new Date(), totalViews: 0, avgViewsPerDay: 0 }
    const rangeInDays = (clickstream[clickstream.length].timestamp - clickstream[0].timestamp)/(1000*60*60*24); 
    
    userSummary.start = new Date(clickstream[clickstream.length].timestamp);
    userSummary.end = new Date(clickstream[0].timestamp)
    userSummary.totalViews = clickstream.length;
    userSummary.avgViewsPerDay = clickstream.length / rangeInDays;
  
    clickstream.map((clicks, index) => {
