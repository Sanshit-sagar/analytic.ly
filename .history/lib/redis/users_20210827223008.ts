...
export async function getSummaryForUser(email: string): Promise<any> {
    ...
    const userSummary: any = {
        rangeInDays: (clickstream[clickstream.length - 1].timestamp - clickstream[0].timestamp)/(1000*60*60*24),
        start: new Date(clickstream[clickstream.length].timestamp),
        end: new Date(clickstream[0].timestamp),
        totalViews: clickstream.length,
        // avgViewsPerDay: clickstream.length/(clickstream[clickstream.length].timestamp-clickstream[0].timestamp)/(1000*60*60*24),
    };
    return userSummary;
}
