...
export async function getClickstreamOnDate(date: Date, isAsc: boolean): Promise<any[] | any> {
    ...
    // let tsStartDate = new Date(timestampStart).toLocaleString()
    ...
    const clicksOnDate = await redis.zrangebylex('clickstream.chronological', `[${timestampStart}`, `(${timestampEnd}`);
    // return clicksOnDate
    // return `Getting clicks from ${timestampStart} to ${timestampEnd}`
