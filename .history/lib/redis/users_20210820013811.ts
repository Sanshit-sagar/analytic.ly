...
export async function getUniqueVisitorsForUser(email: string, limit?: number): Promise<any>  {
    ...
    const firstSeenAt: Map<string, number> = new Map<string, number>();
    ...
    details.map(function(detail, i) {
        ...
        if(!firstSeenAt.has(clickEvent.slug)) {
            firstSeenAt.set(clickEvent.slug, clickEvent.timestamp);
        }
        f
    });
    ...
    return {
        ...
        rankings,
        firsts: Object.entries(firstSeenAt)
    }
}
