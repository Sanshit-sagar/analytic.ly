...
export async function getUrchinsForUserForCategory(email: string, utmCategory: string): Promise<string[]> {
    let urchins: {id: string; name: string; frequency: number; updatedAt: Date}[] = []
            ...
            .exec((_error: any | null, result: any) => {
                ...
                temp.map((res: string,index: number) => {
                    urchins.push({
                        ...
                        updatedAt: new Date(new Date().getFullYear(), new Date.getFullMonth(), new Date.getFullDay() - i) 
                    });
                })
            });
        return Promise.resolve(urchins); 
