...
export async function getUniqueVisitorsFromSlugs(userSlugsWithScores: any[]): Promise<any> {
    ...
    userSlugsWithScores?.rankings?.map((uu, _) => {
        slugsToFetch.push(uu.title);
    }) ?? [];

    let userUniquesBySlug: Map<string, number> = new Map<string, number>();
    let totalUniqueViews: number = 0;

    slugsToFetch.map(async function(stf: string, _) {
        let uniqueViews = await redis.zcard(`slug.${sanitizeSlug(stf)}.clickcount`);
        console.log(`${stf}: ${uniqueViews}`); 
        userUniquesBySlug.set(stf, uniqueViews);
        totalUniqueViews += 1; 
    }); 

    slugsToFetch.map(function(stf: string, _) {
        console.log(`${stf} has ${userUniquesBySlug.get(stf)} views`);
    })
    console.log(`**************${totalUniqueViews}`); 
    return { userUniquesBySlug, totalUniqueViews };
}
