...
export async function getViewsForUser(email: string) {
    return await redis.zrange(`user.${email}.clickcount`, 0, -1)
}
...
export async function getUniqueViewsForUser(email: string): Promise<number> {
    return await redis.zcard(`user.${email}.clickcount`)
}
...
export async function getUniqueVisitorsForUser(email: string, limit?: number): Promise<any>  {
    ...
    const rankingsByFreq: string[] | ScoredSlug[] = formatAndSortSlugs(slugs, limit); 
...
export async function getFrequenciesByCategoryForUser(email: string, category: string): Promise<SlugRankings> {
    ...
    let rankings: string[] | ScoredSlug[] = formatAndSortSlugs(frequencies); 
...
// interface UserSummary {
    ...
    // avgViewsPerSlug: number;
    // dayWithMostViews: number;
    // slugWithMostViews: number;
    // dayWithMostUniques: number;
    // slugWithMostUniques: number; 
    // avgUniqueToViewsRatio: number;
    // avgVisitsPerVisitor: number;
    // avgChangePer: PeriodSet;
    // biggestChangesPer: PeriodSet; 
// };
// 
export async function getSummaryForUser(email: string): Promise<any> {
    const clickstream: any[] = await getClickstreamForUser(email)
    clickstream.sort((a: any,b: any) => parseInt(a.timestampRaw) - parseInt(b.timestampRaw))

    let rangeStart = parseInt(clickstream[0].timestampRaw)
    let rangeEnd = parseInt(clickstream[clickstream.length-1].timestampRaw)
    let res =  new Date(rangeStart);
    let red = new Date(rangeEnd);

    const userSummary: any = {
        start: `${res.getDate()}/${res.getUTCMonth()} ${res.getHours()}:${res.getMinutes()}`,
        end:  `${red.getDate()}/${red.getUTCMonth()} ${red.getHours()}:${red.getMinutes() < 10 ? `0${red.getMinutes()}` : red.getMinutes()}`,
        numDays: (rangeEnd - rangeStart)/(1000*60*60*24),
        totalViews: clickstream.length,
        avgViewsPerDay: clickstream.length/((rangeEnd-rangeStart)/(1000*60*60*24)),
    };
    return userSummary;
}

export async function getStatisticsForUser(email: string): Promise<{ name: string; result: any }[]> {
    // validate email here --> email
    let userStatistics: { name: string; result: any }[] = []; 
    let statisticNames: string[] = ['Total Page Views', 'Total Unique Visits', 'Most Viewed Page', 'Most Frequent Visitor']

    try {
        const multiPromise = await redis.multi()
            .llen(`clickstream.user.sanshit.sagar@gmail.com`)
            .scard(`unique.visitors.for.user.sanshit.sagar@gmail.com`)
            .zrevrange(`user.sanshit.sagar@gmail.com.clickcount`, 0, 0, 'WITHSCORES')
            .zrevrange(`ip.by.user.sanshit.sagar@gmail.com`, 0, 0, 'WITHSCORES')
