...
const getUserData = () => {
    ...
    utmCategories.map((utmCategory: string) => utmParameters.se(utmCategory, []))
    utmCategories.put('source', sources?.length ? [...sources] : [])
    utmCategories.put('medium', mediums?.length ? [...mediums] : []);
    utmCategories.put('term', terms?.length ? [...terms] : []);
    utmCategories.put('campaign', campaigns?.length ? [...campaigns] : []);
    utmCategories.put('content', contents?.length ? [...contents] : []);
        },
        custom: {} 
    }
