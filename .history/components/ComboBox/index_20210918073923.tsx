...
const setSaveItemKeyBuffer = atom<string | undefined>(undefined)
const setSaveItemValueBuffer = atom<string | undefined>(undefined)

const setSaveItemBuffer = atom(
    (get) => {
        return !!get(saveItemBuffer)
    },
    (_get, set, update: string) => {
        set(saveItemBuffer, update)
    }
)
...
export const AriaComboBox = ({ key, label, datumAtom, index, initItems }: ICustomComboBoxProps) => {
    ...
    let onSave = (urchinKey: string, urchinValue: string) => {
        setSaveItemKeyBuffer(urchinKey)
        setSaveItemValueBuffer(urchinValue)
    }
