...
import { Item, ComboBox as ComboBoxComposition } from '../../compositions/ComboBox'
...
const UrchinComboBox = ({ key, utmCategory, utmCategoryAtom, index, initOptions }: IUrchinComboBoxProps) => {
    ...
    let optionList: IItem[] = initOptions?.length ? [...initOptions.map((opt: string, index: number) => { 
        return { id: index, name: opt.name, icon: opt.icon, usedBy: opt.usedBy, viewedBy: opt.viewedBy, updatedAt: opt.updatedAt  }
    })] : []
            ...
            {(item: IItem) =>(
                <Item textValue={item.name}>
                    ...
                    <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1'}}>
                        <Text size='1' css={{ color: '$funkyText' }}> {item.name} </Text>
                        <Text css={{ fontSize: '7.5px', css: '$text'}}> {describe(item)} </Text>
                    </Flex>
                </Item>
            )}
        </ComboBoxComposition>
