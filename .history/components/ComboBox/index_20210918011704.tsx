...
import { useAtom, WriteableAtom } from 'jotai'
...
interface ICustomComboBoxProps {
    key: number;
    label: string; 
    datumAtom: WriteableAtom<string, React.SetStateAction<string>>; 
    index: number;
    initOptions: string[];
}
...
export const ComboBox = ({ key, label, datumAtom, index, initOptions }: ICustomComboBoxProps) => {
    ...
    const [value, setValue] = useAtom<WriteableAtom<string, React.SetStateAction<string>>(datumAtom)
