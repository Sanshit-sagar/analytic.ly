...
import { 
    useUniques, 
    useClickHistoryForSlug 
} from '../../hooks/useClickHistoryForUser'
...
const SlugSelector = ({ slug, updateSlug }: ISlugSelectorProps) => {
    ...
    // call Selectio Group from here if slug details need to be specified
    return (
...
const RangeSelector = () => {
    ...
    const { data, loading, error } = useClickHistoryForSlug(amount, range, interval); 
    ...
    return (
        <Flex css={{ fd: 'row', jc: 'center', ai: 'center' }}>
           ...
           />
            ...
            <Text size='1'> 
                {`[${data?.numClicks} / ${data?.numPeriods}]`} 
            </Text>
        </Flex>
    )
}
