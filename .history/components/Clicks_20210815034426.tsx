...
const useAggregatedClicks = ({ endpoint }) => {
    const { data, error } = useSWR(endpoint, fetcher);

    return {
        timeseries: data.aggregatedTimeseries.timeseries,
        loading: !data && !error,
        error
    }
...
const Clicks = ({ clicksEndpoint = '/api/metrics/clickstream/recent/24/hours' }) => {
    const { clicks, loading, error }: IClicksProps = useAggregatedClicks(clicksEndpoint); 
