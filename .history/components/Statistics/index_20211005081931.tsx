...
export const CachedStatistics = () => {
    ...
    const has = (category: string): boolean => (map.filter(([key, value]: [key: string, value: number]) => key===category).length !== 0)
    const get = (category: string): string => map.filter(([key, value]: [key: string, value: number]) => key===category)[0].value
    const set = (category: string, value: number): void => positions.set(String(category), Number(value))
    const increment = (category: string): void => positions.set(String(category), positions.has(category) ? positions.get(category) + 1 : 1)
