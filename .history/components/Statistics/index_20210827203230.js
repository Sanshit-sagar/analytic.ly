...
const AnimatedNumericalStat = ({ index, name})
        ...
        <Box css={{ padding: '$1', margin: '$1' }}>
            <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1'}}>
                {statistics.map((stat, i) => {
                    return (
                        <StatsWrapper>
                             {i<2 <AnimatedNumericalStat index={i} name={stat.name} />
                                
                               ? <AnimatedStat result={stat.result} /> :
                                    
                                        <Text size='2' css={{ color: '$accent' }}> {stat.result[0]} </Text>
                                        <AnimatedStat result={parseInt(stat.result[1])} />
                                    </Flex>
                                }
                                </>
                            </Flex>
