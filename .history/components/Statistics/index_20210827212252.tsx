...
const AnimatedListedStat = ({ index, name, value, leader = '' }: IStats) => {

    const isNumerical = index < 2;

    return (
        <>
            {isNumerical ? <Text size='3'> {statIcons[index]} {name} </Text> :   
                <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'flex-start', gap: '$1'}}>
                    <Text size='4'> {name} </Text>
                    <Text size='2'> {leader} </Text>
                </Flex>
            } <AnimatedStat result={parseInt(value)} />
        </>
    )
}
