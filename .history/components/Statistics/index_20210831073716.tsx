...
const Statistics = () => {
    ...
    let freqKeys = freqs?.length ? Object.entries(freqs)[currentFreqMapIndex] : []

    useEffect(() => {
        if(freqKeys?.length !currentCategory?.length && !currentFreqs?.length) {
    ...
    const displayNext = () => {
        ++currentFreqMapIndex
        currentFreqMapIndex %= freqKeys.length;
        currentCategory = freqKeys[currentFreqMapIndex][0]; 
        currentFreqs = freqKeys[currentFreqMapIndex][1];
    }
