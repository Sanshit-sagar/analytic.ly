...
export const CachedStatistics = () => {
    ...
    return (
        <Container>
            {freqs && Object.entries(freqs).map( (key: string, value: INormalizedFreqProps[][], i: number) => {
                return value[1].map((itemsForCategory: INormalizedFreqProps[], j: number) => {
                    let leader: INormalizedFreqProps = itemsForCategory[0]
                    return (
                        <Statistic key={`statistic-for-${j}`}>
                            <AnimatedStat 
                                index={j} 
                                name={`Most Used ${key}`} 
