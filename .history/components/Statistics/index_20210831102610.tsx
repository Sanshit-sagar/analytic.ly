    ...
    return (
        <>
            <Flex css={{ width: '300px', fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1'}}>
                ...
                <Text as='span' size='1' css={{ color: '$accent2', fontWeight: '500' }}> {leader} </Text>
            </Flex>
...
const entriesMapAtom = atom((get) => get(entriesAtom))
const entriesValueAtIndexAtom = atom((get) =>get(entriesMapAtom)[[get(entriesIndexAtom)])
const entryTitleFromValueAtIndex = atom((get) => get(entriesValueAtIndexAtom)?.title || 'N/A')
const entryScoreFromValueAtIndex = atom((get) => get(entriesValueAtIndexAtom)?.score || -1)

const Statistics = () => {
    const [entriesIndex, setEntriesIndex] = useState<number>(0)
    const [entriesScore] = useAtom(entryScoreFromValueAtIndex)
        ...
        <Box css={{ width: '300px', margin: '$1' }}>
            ...
            <StatsWrapper key={`statistic-for-${entriesIndex}`}> 
                <AnimatedStat 
                    ...
                    name={`Most Used ${Object.entries(freqs)[entriesIndex][0]}--${entriesIndex}--`} 
