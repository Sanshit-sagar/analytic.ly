...
export const CachedStatistics = () => {
    ...
    const has = (category: string): boolean =>(map.entries(([key, value]: [key: string, value: number]) => key===category).length !== 0)
    const get = (category: string): number => {
        map.entries(([key, value]: [key: string, value: number]) => {
            return key===category)[0].value
