...
export const CachedStatistics = () => {
    ...
    const has = (category: string): boolean => {
        return map.entries(([key, value]: [key: string, value: number]) => (key===category) ? true : null)
    }; 

    const get = (category: string): number | null => map.entries(([key, value]: [key: string, value: number]) => return (key===category) ? value : null);
