...
const NumberOne = ({ leader }: { leader: string }) => {
    return (
        <Text as='button' onClick={} size='1' css={{ color: '$accent', fontWeight: '500' }}>
            {leader}
        </Text>
    );
}
...
export const CachedStatistics = () => {
    ...
    const get = (category: string): number => map.filter(([key, value]: [key: string, value: number]) => key===category)[0].value
