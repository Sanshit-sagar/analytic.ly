    ...
    return (
        <> 
            <Flex css={{ width: '300px', fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1'}}>
                <Text size='4' css={{ fontWeight: 'normal' }}>{statIcons[index]} {name} </Text>
                <Text size='1' css={{ color: '$accent2' }}> {leader} </Text>
            </Flex>
...
const AnimatedNumerical = ({ result }: ISpringNumber) => {
    ...
    return <a.h1 style={{ color: darkMode ? 'white' : 'black' }}>{props.result.to(Math.round)}</a.h1>;
}

const Statistics = () => {
    // const handleNextFreqStat = () => alert('yoyoy');
    ...
    if(statsLoading || smLoading || loading) return <Text> loading... </Text>
    if(statsError || smError || error) return <Text> error... </Text>
        ...
        <Box css={{ width: '300px', margin: '$1' }}>
         ...
         {/*   <StatsWrapper key={`statistic-${statistics.length}`}> 
                <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'flex-end', gap: '$1' }}>
                    <button onClick={handleNextFreqStat}> next </button>
                    <AnimatedStat 
                        index={statistics.length} 
                        name={'Most Used Broswer'} 
                        value={freqs['browser'][0].score}
                        leader={freqs['browser'][0].title}
                    />
                </Flex> 
            </StatsWrapper>    */}
        </Box>
