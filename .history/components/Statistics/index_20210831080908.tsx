...
const AnimatedNumerical = ({ result }: ISpringNumber) => {
    ...
    return <a.h1 style={{ color: 'white', fontWeight: 'bold' }}>{props.result.to(Math.round)}</a.h1>;
}
        ...
        <Box css={{ width: '300px', margin: '$1' }}>
            ...
            <StatsWrapper key={`statistic-${statistics.length}`}> 
                <AnimatedStat 
                    ...
                    name={`Most Used ${Object.entries(freqs)[0][0]}`} 
                    value={parseInt(Object.entries(freqs)[0][1][0].score)}
                    leader={`${Object.entries(freqs)[0][1][0].title}`}
                />
            </StatsWrapper>    
        </Box>
    );
