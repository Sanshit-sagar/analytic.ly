...
export const CachedStatistics = () => {
    ...
    const has = (category: string): boolean =>(positions.map(([key, value]: [key: string, value: number]) => returnkey===category).length !== 0)
    const get = (category: string): number =>positions.filter(([key, value]: [key: string, value: number]) => key===category)[0].value
