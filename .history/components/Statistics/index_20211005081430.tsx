...
export const CachedStatistics = () => {
    const [map, positions] = useMap<string, string>(initialValues)
    
    const has = (category: string) => (map.filter((mapItem) => mapItem===category).length !== 0)
    const get = (category: string) => map.filter((mapItem) => mapItem===category)[0]
    const set = (category: string, value: number) => positions.set(String(category), )
