...
import { ScaleLinear, scaleOrdinal, OrdinaSlScale } from 'd3'
...
export const StackedSteams = ({ layers, xScale, yScale, animate, keys }: SteamerProps) => {
    ...
    const colorScale: OrdinalScale<number, number, never> = scaleOrdinal({
        domain: keys,
        range: ['rgba(255,200,200,1.0)', '#fff', '#667', 'rgba(255,100,100,1.0)', 'hsl(252, 56%, 57%)']
    })

    const patternScale: OrdinalScale<number, number, never> = scaleOrdinal({
        domain: keys,
