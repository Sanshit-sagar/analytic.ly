...
const Chart = ({ width = 500, height = 250, xAccessor = (d: Datum) => d.x, yAccessor = (d: Datum) => d.y }) => {
  ...
  const formatTimeLabel = (index: number): string  => {
    return `${(index + 1)%24}`;
  }

  const formatDateLabel = (index: number): string => {
    if(!startDate) return 'loading...';
    return `${(startDate + (index + 1)/24)}`; // todo add month and modulo month 
  }

  const dataA: Datum[] = new Array(48).fill(null).map((_, i) => ({
    key: "Clicks",
    x: `${formatDateLabel(i)} - ${formatTimeLabel(i) < 10 ? `0${formatTimeLabel(i)}` : formatTimeLabel(i)}:00`,
    y: timeseries[i].y,
  }));
