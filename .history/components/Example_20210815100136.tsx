...
const Chart = ({ width = 500, height = 250, xAccessor = (d: Datum) => d.x, yAccessor = (d: Datum) => d.y }) => {
  const [clicksEndpoint, setClicksEndpoint] = useState(`${API_BASE}/${RECENT_CLICKS_PATH}/48/hours`)

  const handleEndpointUpdate = (updatedTimeAndUnits: string) => {
      setClicksEndpoint(`${API_BASE}/${RECENT_CLICKS_PATH}/${updatedTimeAndUnits}`);
  }

  const { timeseries, loading, error }: XYTimeSeries = useTimeseries(clicksEndpoint);

  let startDate: number | undefined = timeseries?.timeseries ? timeseries.timeseries[0]?.date : undefined
  let startMonth: number | undefined = timeseries?.timeseries ? timeseries.timeseries[0]?.month : undefined
  ...
  const dataA: Datum[] = new Array(48).fill(null).map((_, i) => ({
    ...
    y: timeseries?.timeseries.length ? `${timeseries.timeseries[i].y || 0} clicks` : `${0} clicks`
  }));
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center'}}>
          <ControlPanel endpoint={clicksEndpoint} updateEndpoint={handleEndpointUpdate} /> 
      </div>
    </>
  );
};
