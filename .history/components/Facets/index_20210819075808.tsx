        ...
        <>
            <ParamSelector
                ...
                options={loading ? [] : [...uniques?.slice(0,10).map((unique: IUnique, _: number) => 
                        (`${unique.slug.substring(1)}`).toLowerCase())]
                }
                loading={loading}
                error={error}
            />
...
const Facets = () => {
    ...
    const { data, loading, error, endpoint } = useViews(slug, amount, range, interval); 

    if(loading) return <NoDataPlaceholder height={'600px'} width={'525px'} margin={DEFAULT_MARGIN} />
    if(error) return <p> error... </p>
    if(!data) return <NoDataPlaceholder height={'600px'} width={'525px'} margin={DEFAULT_MARGIN} />
