...
interface IntervalProps {
    ...
    checkValidity: (index: number):boolean;
    selectionOptions: SelectionOption[];
}

const DISABLED: string = 'DISABLED_ITEM';
...
const SelectMenu = ({ 
    ...
    const [selectedItemValidity, setSelectedItemValidity] = useState(true);

    const toggleSelectOpen = () => setSelectOpen(!selectOpen);
    
    const handleSelection = (updatedIndex: number) => {
        const isValid = checkValidity(updatedIndex);
        setSelectedIndex(updatedIndex);
        setSelectedItemValidity(isValid);
    }
                        ...
                        <SelectRadioGroup>
                            {selectionOptions.map((option: SelectionOption, index: number) => {
                                return (
                                    ...
                                    >
                                        <Text size='1'> 
                                            {selectedItemValidity ? option.label : DISABLED}
                                        </Text> 
                                    </SelectRadioItem>
                                )
                            })}
