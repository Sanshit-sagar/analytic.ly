...
const SelectMenu = ({ 
    ...
    const [selectedIndex, setSelectedIndex]: { selectedIndex: number; setSelectedIndex: React.SetStateAction<number> } = useAtom(selectedIndexAtom); 
                    ...
                    <SelectTrigger>
                        <Box  css={{  width: '100%', padding: '$1', bc: 'white' }}>
                            <Flex css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'stretch', gap: '$3' }}>
                                <Text size='1'> {selectionOptions[selectedIndex]} </Text>
                                <ChevronDownIcon />
                            </Flex>
                        </Box>
                    </SelectTrigger>
