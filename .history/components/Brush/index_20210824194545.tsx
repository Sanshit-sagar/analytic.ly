...
const tickSizeOptions: SelectionOption[] = [
    { id: 0, label: 'second', isValid: checkValidity('second') },
    { id: 1, label: 'minute', isValid: checkValidity('minute') },
    { id: 2, label: 'hour', isValid:  checkValidity('hour') },
    { id: 3, label: 'day', isValid: checkValidity('day') },
    { id: 4, label: 'week', isValid: checkValidity('week') },
    { id: 5, label: 'month', isValid: checkValidity('month') },
];

const TickSize = () => {
    const [timeAgo] = useAtom(timeAgoAtom) 

    const checkValidity = (tickSizeStr: string): boolean => {
        const [timeAgo] = useAtom(timeAgoInMsAtom)
        const tickSizeMs = millify(tickSizeStr); 
        return timeAgo >= tickSizeMs; 
    }
