...
const timeAgoInMsAtom: Atom<number | undefined> = atom((get) => getValueInMillis(timeAgoOptions[get(timeAgoAtom)].label))
const timeAgoInMsWithQuantityAtom: Atom<number> = atom((get) => (get(timeAgoInMsAtom) || 1) * (get(quantityAtom) || 1))
const tickSizeInMsAtom: Atom<number | undefined> = atom((get) => getValueInMillis(tickSizeOptions[get(tickSizeAtom)].label))

const invalidTickSize: Atom<boolean> = atom((get) => {
    return ((get(tickSizeInMsAtom) || new Date().getTime()) > get(timeAgoInMsWithQuantityAtom));
});

const numIntervals: Atom<number> = atom((get) => {
    return get(invalidTickSize) ? -1 : (get(timeAgoInMsWithQuantityAtom)/(get(tickSizeInMsAtom) || 1))
}); 
