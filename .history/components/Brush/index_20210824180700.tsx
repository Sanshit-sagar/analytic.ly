...
const TickSize = () => {
    ...
    return (
        <SelectMenu 
           ...
           disabledKeys={tickSizeOptions.filter((option: SelectionOption, index: number) => {
               const currTickSizInMs = millify(option.label);
               return currTickSizInMs>timeAgo) || invalidate(currTickSizInMs, quantify(quantity, timeAgo));
           })}
        />
     )
}
