...
const timeAgoInMsAtom = atom((get) => getValueInMillis(timeAgoOptions[get(timeAgoAtom)]))
const timeAgoInMsWithQuantityAtom = atom((get) => get(timeAgoInMs) * get(quantityAtom))
const tickSizeInMsAtom = atom((get) => getValueInMillis(tickSizeAtom))


const Output = () => {
    const [startTimeAgo] = useAtom(timeAgoAtom)
    const [tickSize] = useAtom(tickSizeInMsAtom)

    return (
        <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'center', gap: '$1'}}>
             <Text size='1'> Start from {} </Text>
...
const Timeseries = () => {
    const [timeAgo] = useAtom(startInMillis)
    const [timeAgoQuant] = useAtom(timeAgoInMsWithQuantityAtom)
    const [endTimeAgo] = useAtom(tickSizeInMsAtom)
