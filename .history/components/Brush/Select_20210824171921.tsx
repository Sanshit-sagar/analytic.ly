...
interface IntervalProps {
    ...
    isValidSelection: (selectedIndex: number | undefined) => boolean = (selectedIndex) => true;
}
                    ...
                    <SelectTrigger>
                        <Box  css={{  width: '100%', padding: '$1', bc: 'white' }}>
                            <Flex css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'stretch', gap: '$3' }}>
                                <Text size='1'  css={{ textTransform: 'uppercase'}}> 
                                    {selectionOptions[selectedIndex || 0].label} 
                                </Text>
                                <ChevronDownIcon />
                            </Flex>
                        </Box>
                        ...
                        <SelectRadioGroup>
                            {selectionOptions.map((option: SelectionOption, index: number) => {
                                return (
                                    ...
                                    >
                                       <Text size='1'> 
                                        {isValidSelection(index) ? option.label} </Text> 
                                    </SelectRadioItem>
                                )
                            })}
                        </SelectRadioGroup>
