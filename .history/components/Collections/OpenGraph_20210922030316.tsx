...
export const OgResultsData = ({ hybridGraph, hybridGraphKeys }: IOpenGraphProps) => (
    <Tree name={hybridGraphKeys?.length ? hybridGraph[hybridGraphKeys[0]] : 'Open Graph'} defaultOpen>   
        {hybridGraphKeys?.filter((k: string) => hybridGraph[k]!==null).map((key, i) => (
            <> {sanitizedResultsLine(key, hybridGraph[key]).length==0 
                ?    <Tree key={i} name={`${key}: ${hybridGraph[key]}`} /> 
                :   (<Tree key={i} name={`${key}`} defaultOpen>
                        <Text key={key}> 
                            {hybridGraph[key]} 
                        </Text> 
                    </Tree>)
