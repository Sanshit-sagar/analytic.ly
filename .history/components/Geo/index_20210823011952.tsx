...
const HeatedGeo = () => {
    ...
    Object.entries(geodata).splice(0, 10).map((entry: [string, GeoDatum], _: number) => {
        let currGeoDatum: GeoDatum = {
            hash: entry[0],
            cfRay: entry[1].cfRay,
            city: entry[1].city,
            country: entry[1].country,
            postalCode: entry[1].postalCode,
            metroCode: entry[1].metroCode,
            timestamp: entry[1].timestamp,
            timezone: entry[1].timezone,
            count: entry[1].count,
            coordinate: entry[1].coordinate
        }; 
        data = data?.length ?  [...data, {...currGeoDatum}] : [{...currG}]
    });

    data.map((d: [string, GeoDatum], i: number) => {
        markers = [...markers, {...d[1].coordinate}];
    });
