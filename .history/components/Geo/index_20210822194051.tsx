...
const HeatedGeo = () => {
  const [mode, _] = useState<string>('less')
  const [fetchStats, _] = useState<boolean>(false);
  ...
  data.map((d, i) => {
    ...
    if(data[i] && data[i][0] && data[i][1]) {
      ...
      const rankedSlug: RankedSlug = {
          ...
          coordinate: {
            x: parseInt(data[i][1].coordinate?.latitude),
            y: parseInt(data[i][1].coordinate?.longitude),
            latitude: parseFloat(data[i][1].coordinate?.latitude),
            longitude: parseFloat(data[i][1].coordinate?.latitude),
