...
const CustomMercator:React.FC<GeoMercatorProps> = ({ 
    ...
    let countryFreqs = geodata.da

    let numCountries = loading || error ? 0 : geodata ? Object.keys(geodata?.countryFreqs).length : 0
    let numCities = loading || error ? 0 : geodata ? Object.keys(geodata?.cityFreq).length : 0
    let numTimezones = loading || error ? 0 : geodata ? Object.keys(?.timezoneFreq).length : 0
