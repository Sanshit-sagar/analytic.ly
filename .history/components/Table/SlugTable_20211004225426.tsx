...
export const TabulatedSlug = ({ data }: TabulatedSlugProps) => {
    const [selectedKey, setSelectedKey] = React.useState('Slug1')

    const list: = useSingleSelectListState<TabulatedSlug>({
        items: [{ name: 'Slug1' }, { name: 'Slug2' }],
        delectedKey: selectedKey,
        onSelectionChange: (key: Key) => setSelectedKey(key),
        getKey: (item: Item) => item.name
    }: SingleSelectListProps<TabulatedSlug>); 
