...
const Table: React.FC<TableProps<T>> = (props: TableProps<T>) => {
    ...
    let list = useAsyncList<Item, C = string>({
        ...
        async sort<Item, C>({ items, sortDescriptor }: { items: Item[]; sortDescriptor: SortDescriptor }) {
            return {
    ...
    let rows = list && list?.items.map((item: Item[], index) => {
        return {
            id: index,
            slug: item['slug'],
