...
import { Checkbox } from '../../../compositions/Checkbox'

import { AriaCheckboxProps, AriaLabelsType, ToggleProps, ToggleState, CheckboxAria } from '../interfaces'
...
export function Table(props: any) {
    ...
    return (
        <table 
            ref={ref} 
            {...gridProps} 
            style={{ borderCollapse: 'collapse', backgroundColor: '$neutral' }}
        >
            <TableRowGroup 
                type='thead'
...
const TableCheckboxCell = ({ 
    ...
    let {   checkboxProps }: { checkboxProps: AriaCheckboxProps } = useTableSelectionCheckbox({ 
        key: cell?.parentKey || `${Math.random()}` 
    }, state)

    // type AriaPropsType = 'aria-labeledby' | 'aria-describedby' | 'aria-checked' | 'aria-label' 
    type ToggleProps = Pick<AriaCheckboxProps, AriaLabelsType>    
    const toggleProps: ToggleProps = checkboxProps

    let inputRef: React.RefObject<HTMLInputElement> = useRef(null)
    let { inputProps } = useCheckbox(checkboxProps, useToggleState(toggleProps), inputRef)
    ...
    return (
        ...
        >
            <Checkbox 
                checked={checkboxProps.defaultSelected ? undefined : checkboxProps.isSelected}
                onCheckedChange={checkboxProps.defaultSelected ? undefined : checkboxProps.onChange}
                defaultChecked={checkboxProps.defaultSelected}
                name={checkboxProps.name}
                value={
                        typeof checkboxProps.value==='number'                 ?   `${checkboxProps.value}` 
                    :   (typeof checkboxProps.value).startsWith('readonly')   ?   String(`${checkboxProps.value}`)  
                    :   typeof checkboxProps.value==='string'                 ?     `${checkboxProps.value}` 
                    :   typeof checkboxProps.value==='boolean' && (checkboxProps.defaultSelected || checkboxProps.isSelected) 
                        ? 'true' 
                        : 'n/a'
                } 
