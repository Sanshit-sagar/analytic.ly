...
import { useHover } from '@react-aria/focus'
...
function TableRow({item, children, state}) {
    ...
    const [pointerType, setPointerType] = useState<PointerType>('none')
    const { isHovered, hoverProps } = useHover({
        onHoverStart: ((event: FocusEvent) => setPointerType(event.pointerType)),
        onHoverEnd: ((_event: FocusEvent) => setPointerType('none'))
    });
    ...
    return (
        <tr
            style={{
                ...
                background: isSelected ? 'blueviolet' : isHovered ? colors.accentHover : 'transparent',
                color: isSelected ? 'white' : isHovered ? colors.loContrast : colors.text,
                outline: isFocusVisible ? '2px solid orange' : 'none'
            }}
