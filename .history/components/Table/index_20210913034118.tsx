...
const SortIcon = ({ isSortedAsc = false, isSortedDesc = false }: ISortIconProps) => (
    isSortedAsc ? <TriangleUpIcon /> : isSortedDesc ? <TriangleDownIcon /> : null 
);
                    ...
                    {headerGroups.map((headerGroup, _) => (
                        <TableHeader {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map((column: any, hgIndex: number) => (
                                ...
                                >
                                    <Flex css={{ width: '100%', fd: 'column', jc: 'space-between', ai: 'center', gap: '$2' }}>
                                    <>  {column.render('Header')} </>
                                    <SortIcon 
                                        isSortedAsc={column.isSorted} 
