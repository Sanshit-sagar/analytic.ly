...
const DataTable = ({ 
    ...
    let status = 
    let progress = {`${(pageIndex-1)*pageSize} to 
    ...
    return (
        <>
            <Text>
                status: {loading ? 'loading' : refreshing ? 'refreshing' : finishing ? 'finishing' : 'idle'}
                showing page(s) {`${(pageIndex-1)*pageSize} to ${pageIndex}*pageSize}`}{" "}
                issue(s){" "}
                <button 
