...
type FooTrueSKM = StronglyKeyedMap<Item, keyof Item, string>;

type ItemKey = OptionsFlags<RowKeys>; 
...
interface ItemsType { [key: ItemKey]: string; };
...
const Table: React.FC<{}> = (props: any) => {
    ...
    let list = useAsyncList({
        async load({ signal, cursor }) {
            ...
            let json: { data: ItemsType[] } = await res.json();
            return {
                items: json.data,
                cursor: `${parseInt(`${cursor}`) + 1}`
            };
        },
        async sort({items, sortDescriptor}: { items: ItemsType[]; sortDescriptor: SortDescriptor }) {
            return {
                items: items && items.sort((a: ItemsType, b: ItemsType) => {
