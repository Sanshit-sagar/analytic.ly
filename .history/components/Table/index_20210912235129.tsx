...
const DataTable = ({ 
    ...
    const paginationButtons = [
        { key: 0, id: 'first',loading: loading, disabled: loading || atEarliest, onClick: firstPage, icon: <ChevronLeftIcon /> },
        {  key: 0, id: 'prev',loading: loading, disabled: loading || atEarliest, onClick: previousPage, icon: <ChevronLeftIcon /> },
        { key: 0,  id: 'next',loading: loading, disabled: loading || atLatest, onClick: nextPage, icon: <ChevronRightIcon /> },
        {  key: 3, id: 'last',loading: loading, disabled: loading || atLatest, onClick: lastPage, icon: <ChevronLeftIcon /> },
        { key: 4, id: 'refresh', loading: loading || refreshing, disabled: refreshing || !doMutate, onClick: mutatePages, icon: <RefreshIcon /> }
    ]
        ...
        <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1'}}>
            <Toolbar>
                <ToolbarToggleGroup type='single' defaultValue='next'>
                    {paginationButtons.map((paginator: IPaginationButtonProps, idx: number) => (
                        <PaginationButton
                            key={idx}
                            value={paginator.id}
