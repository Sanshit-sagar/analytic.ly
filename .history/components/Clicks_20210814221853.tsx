...
const Click = ({ click, index }: IClickProps) => {
    ...
    return <p> {JSON.stringify(click)} </p>
        // <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', margin: '2.5px 0 2.5px 0' }}> 
        //     <p style={{ marginRight: '5px' }}> {index} </p>
            
        //     {columns.map((datum, _) => {
        //         if(datum[0]==='ip' || datum[0]==='timestamp') {
        //             return <p> {JSON.stringify(datum[1])} </p>
        //         }
