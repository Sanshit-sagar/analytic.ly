...
const PieController = ({ filter, updateFilter }: PieControllerProps) => {
  ...
  return (
        <Toolbar aria-label="Formatting options" css={{ height: 35, bc: 'red' }}>
            <ScrollArea> 
                {/* <ToolbarToggleGroup type="single" aria-label="Text formatting" css={{ ml: 250, display: 'inline-flex', fd: 'row', jc: 'flex-end', bc: 'red' }}> */}
                <ToolbarButtonGroup>
                    {options.map(function(option: any, index: number) {
                        return (
                            <ToolbarButton
                                ...
                                value={filter}
                                css={{ bc: 'transparent', border: '1px solid $border', '&:hover': { borderColor: '$border3' }}}
                            >
...
const PieChart = () => {
    let frequenciesByCategory: freqsForCategory = {};
