...
function PieChart({
  ...
  const name: (d: Datum) => number = (d: Datum) => d.title; 
  ...
  const pieSortValues: (n1: number, n2: number) => number = (nA: number, nB: number) => {
    return parseInt(`${nB.score}`) - parseInt(`${nA.score}`);
  }
        ...
        >
          {(pie) => {
            return pie.arcs.map((arc, index) => {
              const { feature }: ArcDatum = arc.data;
              const [centroidX, centroidY] = pie.path.centroid(arc);
              const hasSpaceForLabel = arc.endAngle - arc.startAngle >= 0.1;
              const arcPath = pie.path(arc);
              const arcFill = getLetterFrequencyColor(feature);
              return (
