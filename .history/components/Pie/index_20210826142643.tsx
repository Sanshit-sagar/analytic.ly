...
import { ScoredSlug } from '../../lib/utils/'
...
// 
// const freqsForCategoryAtom: PrimitiveAtom<any> = atom({})
// const filterAtom: PrimitiveAtom<string> = atom('httpProtocol')
// const activeFreqsAtom: PrimitiveAtom<any[]> = atom((get) => get(freqsForCategoryAtom[get(filterAtom)]))

const PieChart = () => {
    const emptyActiveFreqs: ScoredSlug[] = [];
    const freqsForCategory: Map<string, ScoredSlug[]> = new Map<string, ScoredSlug[]>();

    const [filter, setFilter] = useState('httpProtocol')
    const [activeFreqs, setActiveFreqs] = useState(emptyActiveFreqs); 

    useEffect(() => {
        if(!freqsForCategory.has(filter) && !activeFreqs.length)) {
            setActiveFreqs([...freqsForCategory.get(filter)]);
        }
    }, [freqsForCategory, filter, activeFreqs]);

    const handleFilterUpdate = (updatedFilter: string) => {
        setFilter(updatedFilter); 
        if(freqsForCategory.has(updatedFilter)) {
            setActiveFreqs([...freqsForCategory.get(updatedFilter)]);
        } 
    }
    ...
    Object.keys(freqs).map((category: string, _: number) => {
        freqsForCategory.set(category, [...freqs[category])
    });
