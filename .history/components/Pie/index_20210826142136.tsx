...
const PieChart = () => {
    ...
    useEffect(() => {
        if(!freqsForCategory.has(filter) || (freqsForCategory.has(filter) && !activeFreqs.length)) {
    ...
    const handleFilterUpdate = (updatedFilter: string) => {
        ...
        if(freqsForCategory.has(updatedFilter) && freqsForCategory.get(updatedFilter).length > 0) {
