...
function deserialize(data: SlugDetails): SlugDetails | undefined {
    ...
    let expArr: string[] = []
    let expStart: string, expEnd: string 

    if(data?.expiration) {
        expArr = data.expiration.substring(9).split('&')
        expStart = expArray.length ? expArr[0] : ''
        expEnd = expArray?.length && expArray[1]?.length ? expArr[1].substring(8) : ''
    }
    ...
    return {
        ...
        expiration: expArr.length ? `${expStart} to ${expEnd}` : ''
    }
}
