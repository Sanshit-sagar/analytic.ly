...
const Source = () => {
    ...
    return <Tree name={`Source: ${source}`} level={3} defaultOpen />
}

const Medium = () => {
    const medium = useAtomValue(seoMediumAtom)
    return <Tree name={`Medium: ${medium}`} level={3} defaultOpen />
}

const Content = () => {
    const content = useAtomValue(seoContentAtom)
    return <Tree name={`Content: ${content}`} level={3} defaultOpen />
}

const Term = () => {
    const term = useAtomValue(seoTermAtom)
    return <Tree name={`Term: ${term}`} level={3} defaultOpen />
}
