...
function deserialize(data: SlugDetails): SlugDetails | undefined {
    ...
    if(data?.expiration) {
        ...
        expStart = expArr?.length ?  formatter.format(expArr[0]) : ''
        expEnd = expArr?.length && expArr[1]?.length ? formatter.format(parseInt(expArr[1].substring(6)) : ''
    }
    ...
    return {
        ...
        expiration: doesExpire ? `${expStart} to ${expEnd}` : ''
    }
}
