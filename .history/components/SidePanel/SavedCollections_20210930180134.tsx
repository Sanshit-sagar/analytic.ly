        ...
        <CollectionContainer>    
            <Tree name={<Text size='2'> {name} </Text>} level={1} defaultOpen>
                {data && data.map((datum: string, i: number) => (
                    <Tree key={`slug-${i}`} level={2} name={datum} />
                ))}
            </Tree>
        </CollectionContainer>
    )
            ...
            <CollectionContainer>
                {
                    ...
                    ? <Mapping name={collectionName} data={data} loading={loading} error={error} />
                    : <Listing name={collectionName} data={data} loading={loading} error={error} />
                }
            </CollectionContainer>
        </ScrollArea>
    )
...
export const SavedSlugs = () => {
    ...
    return <CollectionWrapper collectionName={'Slugs'} swrProps={swrProps} /> 
}


export const SavedDestinations = () => {
    const { data, loading, error }: SwrResponse = useSavedDestinations()
    return <CollectionWrapper collectionName={'Destination URLs'} data={data} loading={loading} error={error} /> 
}