...
const makeSelectionAtom = atom(
    ...
    (get, set, update: StoredSlugState) => {
        set(slugSelectionAtom, get(slugSelectionAtom).map((slugState: StoredSlugState, _index: number) => {
            return {
                slug: slugState.slug,
                selectedValue: slugState.slug !== update.slug ? slugState.selectedValue : update.value,
                isSelected: slugState.slug !== update.slug ? slugState.isSelected : update.isSelected,
                isLoading:  slugState.slug !== update.slug ? slugState.isLoading : update.value && update.slug && !update.endpoint ? true : false,
                endpoint:  slugState.slug !== update.slug ? slugtate.endpoint : update.endpoint || ''
            }
        }));
    }
