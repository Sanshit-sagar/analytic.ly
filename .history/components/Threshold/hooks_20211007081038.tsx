...
export const useZippedClicks = () => {
    ...
    let bounds = {
        ...
        end: rBounds?.length && rBounds[0] && rBounds[1] ? new Date(Math.max(...rBounds)) : now
    }

    return {
        bounds,
        dates: [...dateRangeGenerator(bounds.start, bounds.end)]
    }
}

export const useSlugsWithViews = () => {
    let endpoint = `http://localhost:3000/api/users/sanshit.sagar@gmail.com/rankings/frequencies`
