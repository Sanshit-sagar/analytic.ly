...
export const AreaDifference = ({ clicks, height, width, margin = DEFAULT_MARGIN }: AreaDifferenceProps) => {
    ...
    const clicksScale = scaleLinear<number>({
        domain: [
            Math.min(...clicks.map((d: Click) => Math.min(ny(d), sf(d)))),
            Math.max(...clicks.map((d: Click) => Math.max(ny(d), sf(d)))),
        ],
        range: [yMax, 0],
        nice: true,
    }); 
            ...
            <svg height={height} width={width}>
                ...
                <Group left={margin.left} top={margin.top}>
                    ...
                    <Threshold<Click>
                        ...
                        y0={(d: Click) => temperatureScale(groupA(d)) ?? 0}
                        y1={(d: Click) => temperatureScale(groupB(d)) ?? 0}
                        clipAboveTo={0}
                        clipBelowTo={yMax}
                        curve={curveBasis}
                        belowAreaProps={{
