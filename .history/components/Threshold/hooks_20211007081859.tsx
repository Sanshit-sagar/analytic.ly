...
export const useZippedClicks = () => {
    ...
    // let bounds = {
    //     start: !isNullable(lBounds) ? new Date(min(lBounds)) : nowMinusOneYear,
    //     end: !isNullable(rBounds) ? new Date(max(rBounds)) : now
    // }

    return {
        bounds: { lBounds, rBounds, now, nowMinusOneYear },
