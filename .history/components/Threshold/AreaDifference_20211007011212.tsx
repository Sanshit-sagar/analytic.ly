...
const Lines = (endpoint) => {
     ...
     const { data, loading, error } = useAsyncJotai(endpoint?.length ? endpoint : null)

    return <Text> {JSON.stringify(endpoints)} </Text> 
}


export const AreaDifference = ({ endpointAtom, height, width, margin = DEFAULT_MARGIN }: AreaDifferenceProps) => {
