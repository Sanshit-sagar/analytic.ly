    ...
    (get, set, update: SlugState) => {
        set(slugSelectionAtom, {
            ...
            [`${update.slug}`]: {
                ...
                loading:  update.value && update.slug ? true : false,
            }
        });
    }
);
...
const globalFilterStrAtom = (
    (get) => `${get(amountAtom)}/${get(rangeAtom)}/${get(intervalAtom)}`
);
const filteredSlugClicksAtom = (
    (get) => `/api/metrics/slug/${get(slugToFetchAtom)}/tail/${get(globalFilterStrAtom)}`
)
/api/metrics/slug/:slug/tail/:amount/:range/:interval
