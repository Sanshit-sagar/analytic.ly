...
export const useZippedClicks = () => {
    ...
    let bounds = {
        start: lBounds?.length && lBounds[0] && lBounds[1] ? new Date(min(...lBounds) ?? now.getTime()) : nowMinusOneYear,
        end: rBounds?.length && rBounds[0] && rBounds[1] ? (new Date(max(...rBounds)) : now
    }
