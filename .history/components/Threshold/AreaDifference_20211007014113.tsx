...
import { Click, MockView, AreaDifferenceProps } from './interfaces'
...
function formatSlugViews(views: MockView[]): Date[]  {
    let fmtslugs: Date[] = views.map((view, i) => {
        let viewedAt = new Date(`${view.timestamp}`)
        return viewedAt
    });
    return fmtslugs
}
type MockView = { id: number; cfRay: string; timestamp: number; slug: string; user: string; }[]

export const Tester = () => {
    let groupAData: MockView = formatSlugViews(mockViewsA.views)
    let groupBData: MockView = formatSlugViews(mockViewsB.views)
    
    return (
        <Box css={{ height: '200px', width: '200px'}}>
            <Text> {JSON.stringify(groupAData)}</Text> 
        </Box>
    );
}
