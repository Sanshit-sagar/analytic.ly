...
const updateSlugConfigAtom = atom(
    null,
    (get, set, update: { index: number | undefined; value: string; }) => {
        set(slugConfigAtom, [
            ...get(slugConfigAtom).slice(0, config.index - 1), 
               { index: update.index,  value: update.value },
            ...get(slugConfigAtom).slice(config.index + 1)
        ]);
    }
)
