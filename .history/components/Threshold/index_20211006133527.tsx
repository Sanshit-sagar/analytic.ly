...
const INIT_SELECTION_STATE: { [key: string]: StoredSlugState }[] = [
    'slug1': { 
        isSelected: false,
...
const slugFetchEndpointsAtom: WritableAtom<string[], string[]> = atom(
    ...
    (_get, set, update: string[]) => {
        set(slugSelectionAtom, Object.entries(get(slugSelectionAtom)).map((entry: [string, StoredSlugState], i: number) => (
             `${entry[0]}`: {
                ...entry[1],
                isLoading: false,
                endpoint: update[i],
