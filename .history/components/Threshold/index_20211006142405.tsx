    ...
    (get, set, update: StoredSlugState) => {
        set(slugSelectionAtom, get(slugSelectionAtom).map((slugState: StoredSlugState, _index: number) => {
            return {
                ...
                endpoint: slugState.slug`/api/metrics/slug/${slugState.selectedValue}/tail/${get(globalFilterStrAtom)}`
            }
        }))

        set(slugFetchEndpointsAtom, get(slugSelectionAtom).map((slugState: StoredSlugState) => {
