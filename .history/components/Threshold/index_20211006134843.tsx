...
const makeSelectionAtom = atom(
    ...
    (get, set, update: SlugState) => {
        set(slugSelectionAtom, get(slugSelectionAtom).map((slugState: StoredSlugState, index: number) => {
            return {
                slug: slugState.slug,
                selectedValue: slugState.slug !== update.slug ? slugState.value : update.value,
                isSelected: slugState.slug !== update.slug ? slugState.isSelected : update.isSelected && update.slug ? true : false,
