...
const filteredSlugClicksUrlsAtom = atom(
    (get) => {
const slugSelectionAtom = atom(INIT_SELECTION_STATE)
        let urlsForFetching: string[] = get(slugSelectionAtom).map((slugState: { selectedValue: string | undefined; isSelected: boolean; isLoading: boolean; }) => { 
            if(slugState.isSelected && slugState.isLoading) {
                
                `/api/metrics/slug/${get(slugToFetchAtom)}/tail/${get(globalFilterStrAtom)}`
); 
