    ...
    (get, set, update: StoredSlugState) => {
        set(slugSelectionAtom, get(slugSelectionAtom).map((slugState: StoredSlugState, _index: number) => {
            return {
                ...
                endpoint: slugState.slug===update.slug ? `/api/metrics/slug/${update.sele}/tail/${get(globalFilterStrAtom)}`:''
            }
        }))
    }
);
...
const SlugSelector = ({ 
    ...
    const resetSlug = useUpdateAtom(resetSelectionAtom) // pass in slugId as identifier in array of select menus
