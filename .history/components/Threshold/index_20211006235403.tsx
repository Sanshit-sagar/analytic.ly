...
const slugConfigAtom: SlugConfig[] = atom([
    { id: '0', index: 0, value: 'n/a' },
    { id: '1', index: 0, value: 'n/a' },
])

const selectedIAtom = atom(
    (get) => get(slugConfigAtom).value
)
const selectedIndexAtom = atom(
    (get) => get(slugConfigAtom).index
)
const updateSlugConfigAtom = atom(
    null,
    (get, set, update: SlugConfig) => {
        set(slugConfigAtom, get(slugConfigAtom).map((config: SlugConfig, idx: number) => {
            if(idx===update.index) {
                return { 
                    ...update 
                }
