...
const ClickHistory = ({ amount, range, interval }: ClickHistoryProps) => {
    ...
    const mappedData: Datum[] = clicks.map((click: { x: number, y: number }, i: number) => {
        let fmtTimestampObj: { timestamp: number; fmtTimestamp: string } = formatClickDate(click.x, start, interval)
        let timestamp: TimeStamp = fmtTimestampObj.timestamp;
        let clickscore: ClickScore = click.y
        let clickdate: ClickDate = new Date(timestamp)
        let clickfmttime: ClickFmtTime = fmtTimestampObj.fmtTimestamp;
