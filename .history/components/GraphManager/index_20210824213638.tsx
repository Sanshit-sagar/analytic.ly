...
const GraphManager = () => {
    ...
    const [timeAgoInMs] = useAtom(timeAgoInMsAtom)
    const [tickSize] = useAtom(tickSizeInMsAtom)
    const [numIntervals] = useAtom(numIntervalsAtom)
    ...
    return (
        <DashboardDisplayBox>
            <VisxParentSizeWrapper> 
                <Controls />    
                <Text> {quantity} | {timeAgoOptions[timeAgo].label} | {tickSizeOptions[tickSize].label} | {numIntervals} </Text>
                <ClicksData 
                    amount={quantity} 
                    range={timeAgoOptions[timeAgo].label} 
                    interval={tickSizeOptions[tickSize].label} 
                />
            </VisxParentSizeWrapper>
        </DashboardDisplayBox>
    );
