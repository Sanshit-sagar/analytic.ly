...
const getFormatValue = (d: Datum): ClickScore => `<--${d.clickscore}-->`
...
const PrimaryChart: React.FC<PrimaryChartProps> = ({ 
    ...
    const handleTooltip = useCallback((event: TooltipEvent) => {
        ...
        if (d1 && getDate(d1)) {d = x0.valueOf() - getDate(d0).valueOf() > getDate(d1).valueOf() - x0.valueOf() ? d1 : d0}}
                  ...
                  >
                    <ul style={{ padding: '0', margin: '0', listStyle: 'none' }}>
                      <li style={{ paddingBottom: "0.25rem" }}>
                        <b>{customDateFormat(getDate(tooltipData), 'dayhourmin')}</b>
                      </li>
                      <li>
