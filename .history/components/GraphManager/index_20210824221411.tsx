...
const timeAgoStrAtom = atom((get) => timeAgoOptions[get(timeAgoA)].label)
const GraphManager = () => {
    const [quantity] = useAtom(quantityAtom)
    const [timeAgo] = useAtom(timeAgoAtom)
    const [tickSize] = useAtom(tickSizeAtom)
    const [numIntervals] = useAtom(numIntervalsAtom)
    ...
    return (
        <DashboardDisplayBox>
            <VisxParentSizeWrapper> 
                <Controls />    
                <Text> {quantity} | {} | {tickSizeOptions[tickSize].label} | {numIntervals} </Text>
