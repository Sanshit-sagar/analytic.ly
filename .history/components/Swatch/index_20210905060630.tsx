...
interface IThematicaikonProps {
    ...
    theme: ITheme;
    index: number;
    handleRetheme: (i: number) => void;
}

const Thematicaikon = ({ key, isInUse, theme, index, handleRetheme }: IThematicaikonProps) => {
    return (
        <IconButton
            key={`theme-${key}`}
...
const ModeController = () => {
    const activeThemeIndex = useAtomValue(getToggledIndexAtom)

    // <> {activeThemeIdStr.split('-')[1]==='light' ? <SunIcon /> : <MoonIcon />} </>  
    return (
       <Flex css={{ alignItems: 'center' }}>
           <Switch id='darkMode' value={darkMode} onValueChange={re}>
               <SwitchThumb />
           </Switch>
       </Flex>
    );
                ...
                <Flex css={{ fd: 'row', jc: 'flex-start', ai: 'flex-start', flexWrap: 'wrap', width: '200px', height: '100%' }}>
                    {themes.map((theme: ITheme, i: number) => {
                         ...
                         return (
                            <Thematicaikon 
                                key={`theme-no.${i}`} 
                                isInUse={activeThemeIdStr===theme.id}
                                theme={theme}
                                index={i}
                                handleRetheme={handleRetheme}
                            />
                         );
                    })}
                </Flex>
