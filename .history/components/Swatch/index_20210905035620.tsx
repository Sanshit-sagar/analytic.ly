...
const Thematicaikon = ({ key, isInUse, theme, handleRetheme }: IThematicaikonProps) => {
    return (
        <IconButton
            key={`theme-${key}`}
            onClick={() => handleRetheme(key)}
            css={{ 
                bc: theme.color, 
                mb: '$1', 
                mr: '$1', 
        ...
        <Popover>
            ...
            <PopoverContent>
                ...
                <Flex css={{ fd: 'row', jc: 'flex-start', ai: 'flex-start', flexWrap: 'wrap', width: '200px', height: '100%' }}>
                    {themes.map((theme: i, i: number) => (
                         if(i%2===1) return null
                        <Thematicaikon 
                            key={i} 
                            isInUse={activeThemeIdStr===theme.id}
                            theme={theme}
                            handleRetheme={() => handleRetheme(i)}
                        />
                    ))} 
                </Flex>
