...
function Spinner() {
  ...
  const Loading = ({ type,  isIndeterminate, value }) => {
    const label = 'Loading...'
    const value = value && !isIndeterminate ? value : ''

    return type==='spinner' ? <Spinner /> : <ProgressBar label={label} value={value} isIndeterminate={isIndeterminate} /> 
  }