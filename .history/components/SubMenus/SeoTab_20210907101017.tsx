    ...
    return (
        <Label css={{ color: '$text' }}> 
            <Flex css={{  fd: 'row', jc: 'flex-start', ai: 'center', gap: '$1' }}>
                ...
                {Object.keys(seoGroups).map((seoGroup: SeoGroupEventConfig, index: number) => {
                    let culprit = useAtomValue(seoGroup.atom)
                    let eventColor = culprit===seoParam.id ? seoGroup.colors.on : seoGroup.colors.off
                    let eventContent = culprit===seoParam.id ? seoGroup.icon : `${'N/A'}`
                    return <Text css={{ color: eventColor }}> {content} </Text>;
                })}
