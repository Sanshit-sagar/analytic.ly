...
export const suggestionCountAtom = atom(0)

const SuggestedSlug  = () => {
    const setSuggestedSlug = useUpdateAtom(suggestedSlugAtom)
    const setSuggestionCount = useUpdateAtom(suggestionCountAtom)

    const incrementFetchCount = () => setSuggestionCount(suggest)
    ...
    React.useEffect(() => {
        ++fetchCount
        if(!loading && !error) setSuggestedSlug(suggestion)
        else setSuggestedSlug('')
