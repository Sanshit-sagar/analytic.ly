...
const CategorisedResults = () => {
    ...
    const categoriesedUrchins = data.userUrchins
    return (
        <> 
        { loading ? 'loading...' : data?.userUrchins && data.userUrchins[seoParam.id] ? 
            <Box css={{ height: '30px', width: '150px' }}>
                <Text> { JSON.stringify(data.userUrchins[seoParam.id]) } </Text>
            </Box> 
            : 'error...'
        } 
        </>
