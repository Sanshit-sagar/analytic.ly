...
const ParamsList = ({ params, name, index }: IParamsList) => {
    ...
    // useEffect(() => setExpanded(!expanded, [focussed, expanded]); 
    const handleToggleList = () => {
        setExpanded(!expaned)
    }

    return (
        <Box key={index} css={{ border: 'thin solid $border', br: '$2', padding: '$1', margin: '$1', '&:hover': { borderColor: '$border3'} }}>
            <Flex as={button} onClick={handleToggleList} css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1' }}>
