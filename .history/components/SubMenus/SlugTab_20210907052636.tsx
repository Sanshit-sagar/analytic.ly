...
function hashABForControlGroup(a: string, b: string) {
    return `${controlGroupName}-item-${a}-value-${b}`
}
...
const SeperatorSelector = () => {
    ...
    const hash = (a?: string, b?: string, i: number): string => {
        a && b && i ? `${controlGroupName}-hashAB(a,b)}-${hashI(i)}` : a && b ? hashAB(a, b) : hashI(i) 
    };
