...
const SuggestedSlugs = () => {
    const { suggestions, loading, error } = useSuggestedSlugs({ saltLength: 3, numStrings: 2 });

    if(loading) return <Text size='1'> loading... </Text>
    if(error) return <Text size='1'> error! {error.message} </Text>

    return <Text>{JSON.stringify(suggestedSlugs)}</Text> 
    );
