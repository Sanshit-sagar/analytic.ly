...
const SeperatorSelector = () => {
    ...
    const controlGroupName = 'SEPARATOR'

    const isSelected = (i: number) => i===separatorIndex
    const handleSelection = (selectedIndex: number) => setSeparatorIndex(selectedIndex)
    const SelectionIndicator = ({i}:{i:number}) => isSelected(i) ? <SelectIndicator /> : null 
   
   
    const evaluate = (value: number): string => `${index}`
   
    const hash = (a: string, b: string) => `${groupName}-${a}-or-${b}`
