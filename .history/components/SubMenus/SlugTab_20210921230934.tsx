...
const SuggestedSlug  = () => {
    const [suggestedSlug, setSuggestedSlug] = useAtom(suggestedSlugAtom)
    ...
    if(error) return <Text size='2'> error! {error.message} </Text>
    if(!suggestedSlug) return <Text size='4'> No data </Text>

    return (
        <Text size='6' css={{ color: '$text'}}>
            {suggestedSlug?.length}
        </Text>
    )
}
