...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: LoadingState) => ls==='loading' || ls==='loadingMore' || ls==='filtering' || ls==='sorting'
    const isServiceStateLoading = (s: Service) => s.status==='loading' || s.status==='init' 
    ...
    return (
        <ComboBox
            ...
            isLoading={( list.isLoading || loadingState='filtering' || service.status==='loading' || service.status=='init') ? true : false}
            ...
            {(item: IUrchin) => (
                <Item key={item.id}>
                    <UrchinInfoContainer>
                        ...
                        <UrchinTimeAgo> {humanReadable(item.updatedAt)} </UrchinTimeAgo>
                    </UrchinInfoContainer>
                </Item>
            )}
        </ComboBox>
