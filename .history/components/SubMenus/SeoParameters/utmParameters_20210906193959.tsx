...
const ExpandableUtmParameters  = ({ userUtmData }: IExpandableUtmParametersProps) => {
    ...
    return (
        <Accordion 
            defaultValue={focussed} type="multiple">
            {userUtmData.map((userUtm: UserUtmDatum, index: number) => {
                const isFocussed = focussed===userUtm.utm
                const isOpen = false // TODO

                const category = userUtm ? userUtm.utm : null
                const tagsForCategory = userUtm.tags || []
                const numTagsForCategory = tagsForCategory ? tagsForCategory.length : 0
