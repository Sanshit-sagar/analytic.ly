...
function timeAgo(d: Date) {
    ...
    let diff = Math.round((nowTimestamp - agoTimestamp) / (1000 * 60 * 60 * 24))
    return diff<=1 ? `${rnd(diff * 24)} hours ago` 
            : diff <= 7 ? `${rnd(diff)} days ago`
            : diff <= 30 ? `${rnd(diff / 7)} weeks ago`
            : diff <= 356 ? `${rnd(diff / 30)} months ago`
            : `${rnd(diff / 365)} years ago`
}
                ...
                <Item key={item.id}>
                    <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$2'}}>
                        ...
                        <Flex css={{ fd: 'row', jc: 'stretch', ai: 'flex-start', gap: '$1'}}>
                            <Text size='1' css={{ color: '$accent' }}> {timeAgo(item.updatedAt)} </Text>
                        </Flex>
                        <Button 
                            color='transparent' 
                            style={{ 
