...
const hoverDetailsAtom = atom<HoverDetails>(
    ...
    (_get, set, update: HoverDetails) => {
        let { isHovered, hoveredType, hoveredParameter } = update
        
        set(isHoveredAtom, update.isHovered)
        set(hoveredTypeAtom,update.hoveredType)
        set(hoveredParamAtom, update.hoveredParameter)
    }
)

const hoverGroupAtom = atom(
    (get) => !get(isHoveredAtom) || (!get(hoveredParamAtom) && !get(hoveredTypeAtom)) ? undefined : ({ 
        hoverType: get(hoveredTypeAtom), 
        parameter: get(hoveredParamAtom),
    }),
    (get: (atom: Atom<HoverDetails>) => HoverDetails, set, updateEvent: FocusEvent) => {
