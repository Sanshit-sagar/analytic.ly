...
type CategoryType = UrchinCategoryEnum.MEDIUM | UrchinCategoryEnum.SOURCE | UrchinCategoryEnum.TERM | UrchinCategoryEnum.CAMPAIGN | UrchinCategoryEnum.CONTENT | UrchinCategoryEnum.NONE
type HoverType = HoverTypeEnum.START | HoverTypeEnum.END | HoverTypeEnum.CHANGE | HoverTypeEnum.NONE

interface HoverDetails {
    isHovered: boolean;
    hoveredParameter: string | undefined;
    hoveredType: string | undefined; 
}

function mapIdToParameter(id: string): CategoryType {
    let map =[UrchinCategoryEnum.MEDIUM, UrchinCategoryEnum.TERM, UrchinCategoryEnum.SOURCE, UrchinCategoryEnum.CAMPAIGN, UrchinCategoryEnum.CONTENT];
...
const hoverDetailsAtom = atom<HoverDetails>(
    (get) => {
        'isHovered': get(isHoveredAtom),
        'hoveredType': get(hoveredTypeAtom) || undefined,
        'hoveredParameter': get(hoveredParamAtom) || undefined,
    }
)

const hoverGroupAtom = atom(
    (get) => {
        return 
            !get(isHoveredAtom) ? undefined :
            !get(hoveredParamAtom) && !get(hoveredTypeAtom) ? undefined :
            { hoverType: get(hoveredTypeAtom), parameter: get(hoveredParamAtom) }
    },
    (get, set, updateEvent: FocusEvent) => {
        if(updateEvent.type === 'hoverend') {
            set(hoverDetailsAtom, false, HoverTypeEnum.END,  mapIdToParameter(updateEvent.target.id))
        } else {
            set(hoverDetailsAtom, true, HoverTypeEnum.START,  mapIdToParameter(updateEvent.target.id))
        }
        let { isHovered, hoveredType, hoveredParameter } = get(hoverDetailsAtom)
