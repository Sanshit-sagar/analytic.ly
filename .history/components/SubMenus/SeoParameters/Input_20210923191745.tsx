...
export const SeoParamsInput = () => {  
    ...
    useEffect(() => {
        setUrl(user?.primaryEmailAddress?.length ? ` : undefined)
    }, [isLoading])

    useEffect(() => {
        if(isSignedOut) router.push('/signin'), 
    }, [isSignedOut, user, router]);
            ...
            {urchinAtoms.map((atomicUrchin: UrchinAtom) => {
                ...
                return (
                    <UtmParamInput 
                        ...
                        endpoint={`/api/urchins/user/${user?.primaryEmailAddress ?? ''`/${category}`}
                    /> 
                )
            })}
        </UrchinGroupContainer>
