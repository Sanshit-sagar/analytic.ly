...
interface IUtmTagSummary {
    name: string; tags: { name: string }[]; isFocussed: boolean }
}
...
const Summary = ({ name, tags, isFocussed }: IUtmTagSummary) => {
    const summarized = `${name} [${tags.length}]`

    return (
        <Text size='1' css={{ color: isFocussed ? '$funky' : '$text' }}> 
            {summarized} 
        </Text>
    )
}
