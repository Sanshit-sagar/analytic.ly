...
const Details = ({ name, tags, isFocussed, isOpen }: IUtmTagDetailsProps) => {
    ...
    const focussedInput = useAtomValue()

    return (
        <Flex css={{ border: detailsBorder, fd: 'column', jc: 'flex-start', ai: 'flex-start', gap: '$1' }}>
            {tags
                .filter((unfilteredTag: ITag) => unfilteredTag.name.startsWith(focussedInput))
                .map((tag: ITag, i: number) => {
...
const ExpandableUtmParameters  = ({ userUtmData }: IExpandableUtmParametersProps) => {
    ...
    return (
        <>
            <Text> {focussed}: {focussedInput} </Text> 
            <Accordion defaultValue={'campaign'} type="single">
