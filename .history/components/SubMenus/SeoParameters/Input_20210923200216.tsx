...
function timeAgo(d: Date) {
    ...
    return `used ${Math.round((nowTimestamp - agoTimestamp) / (1000 * 60 * 60 * 24))} days ago`;
}
            ...
            {(item: IUrchin) => (
                <Item key={item.id}>
                    <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$2'}}>
                        ...
                        <Button color='transparent' style={{ padding: 0, margin: 0, border: 'thin solid $border', '&:hover': borderColor: '$boder3' }}
                    </Flex>
                </Item>
            )}
        </ComboBox>
