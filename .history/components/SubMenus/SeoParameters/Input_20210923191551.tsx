...
export const SeoParamsInput = () => {  
    ...
    useEffect(() => {
        setUrl(user?.primaryEmailAddress?.length ? `/api/urchins/user/${user?.primaryEmailAddress ?? ''}` : undefined)
    }, [isLoading])

    useEffect(() => if(isSignedOut) router.push('/signin'), [isSignedOut, user, router])
