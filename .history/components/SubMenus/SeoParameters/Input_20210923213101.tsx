            ...
            {(item: IUrchin) => (
                <Item key={item.id}>
                    <Flex css={{ width: '100%', fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$2'}}>
                        ...
                        <Text size='1' css={{ height: '3em', color: '$funkyText', width: '100%', fd: 'row', jc: 'flex-start', ai: 'center', gap: '$1' }}> 
                            <StarIcon /> {Math.round(item.frequency * 100)/10}  <EyeOpenIcon /> {Math.round(item.frequency * 100)/10}
                        </Text>
