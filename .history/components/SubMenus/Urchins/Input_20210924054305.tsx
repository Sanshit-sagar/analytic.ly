...
function NewUrchinFactory(label: UrchinCategoryType, initValue: string, listIndex: number, list: AsyncListData)  {
    const datetime = new Date()
    return {
        key: list.getKey(list.get(listIndex)),
        id: `${listIndex}`,
        category: label,
        name: initValue,
        frequency: 1,
...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    let onSelectionChange = (key: string) => {
        let selectedItem: SavedUrchin = list.items.find((item: SavedUrchin, index: number) => {
            return item.id===key || { ...list.items[0], name: `item-${index}` };
        })

        // TODO: ask if reader wants to replace or append previous filter value if one was there 
        setSelectedKey(list.getKey(selectedItem))
    }
