...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: LoadingState) => (ls===LoadingStateEnum.LOADING || ls===LoadingStateEnum.LOADING_MORE || ls===LoadingStateEnum.FILTERING || ls===LoadingStateEnum.SORTING) 
    const isServiceStateLoading = (s: ServiceState) => (s===ServiceStateEnum.LOADING || s===ServiceStateEnum.INIT) ? true : false
    
    let isComboBoxLoading = () => isLoadingStateLoading(list.loadingState) || isServiceStateLoading(service.status)
