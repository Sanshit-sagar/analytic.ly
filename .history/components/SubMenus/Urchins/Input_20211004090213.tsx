...
export const UtmParamInput = ({ key, label, atom, endpoint }: IUrchinListProps) => { 
    const [filterValue, setFilterValue] = const [value, setValue] = useAtom(atom)
    ...
    const handlePersist = async () => {
        ...
        let postEndpoint = `/api/urchins/user/sanshit.sagar@gmail.com/${label}/${newUrchin.name}`

        let response = await fetch(postEndpoint, postOptions)
            ...
            .catch((error) => {
                console.error(error)
                toast.error(`Couldn't add ${newUrchin.name} to ${label}s. Try again?`)
            })
    }
    ...
    const handleDelete = async (itemKey: Key) => {
        ...
        let response = await fetch(deleteEndpoint, deleteOptions)
            .then(async (response) => await response.json())
            .then((result) => {
                if(result.data) {
                    list.remove(itemKey)
            ...
            .catch((error: any) => {
                ...
                toast.error(`Couldn't delete ${toDelete.name}. Try again?`)
            });
    } 
