...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: string) => (ls===LoadingStateEnum.LOADING || ls===LoadingStateEnum.LOADING_MORE || ls===LoadingStateEnum.FILTERING || ls===LoadingStateEnum.SORTING) ? true : false; 


    const isServiceStateLoading = (s: string) => s===ServerStateEnum.LOADING || s==='init';
    ...
    let onSelectionChange = (key: string) => {
        let selectedItem: NewUrchin = list.items.find((item: NewUrchin) => item.id===key)
        setSelectedKey(key)
    ...
    const handlePersist = () => {
        const newUrchin: NewUrchinProps = NewUrchinFactory(label, filterValue, list.items?.length || 0)
