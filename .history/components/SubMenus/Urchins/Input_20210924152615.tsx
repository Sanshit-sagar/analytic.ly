...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    <>
                ...
                <Item key={item.id}>
                    <UrchinInfoContainer>
                        ...
                        <UrchinTimeAgo> 
                            ...
                            <DeleteButton itemName={item.name} onClick={handleRemove}>
        ...
        {service.state.loading && <Text> loading... </Text>}
        {error && <Text> Error!! </Text>}
        {service.state.response && <Text> response: {JSON.stringify(response)} </Text>}
    </>
    )
}
