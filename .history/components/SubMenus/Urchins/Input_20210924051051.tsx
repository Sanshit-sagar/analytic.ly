...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    let { service, mutate } = useUrchinMutator()
    ...
    let filteredItems: SavedUrchin[] = list.items.filter((item: SavedUrchin) => {
    ...
    let onSelectionChange = (key: Key) => {
        let selectedItem: SavedUrchin = list?.items?.find((item: SavedUrchin) => {
            return item ? item.id === key : null; 
        });
        // TODO: ask if reader wants to replace 
        // or append previous filter value if one was there 
        setFilterValue(selectedItem?.name || undefined)
        setSelectedKey(key)
    };
