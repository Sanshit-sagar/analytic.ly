...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => { 
    ...
    const handleDelete = async (itemKey: Key) => {
        ...
        let response: DeleteApiResponse = await fetch(deleteEndpoint, {
            method: 'DELETE',
            headers: jsonHeaders,
        })
        ...
        if(response.data===true) {
            list.remove(itemKey)
            toast.success(`Successfully deleted ${toDelete.name}. ${JSON.stringify(response)}`)
        } else {
            toast.error(`Unable to delete ${toDelete.name}, Try Again?.  ${response.message}`)
        }
    }; 
