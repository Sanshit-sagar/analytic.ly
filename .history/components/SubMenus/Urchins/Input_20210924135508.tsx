...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: string) => (ls===LoadingStateEnum.LOADING || ls===LoadingStateEnum.LOADING_MORE || ls===LoadingStateEnum.FILTERING || ls===LoadingStateEnum.SORTING) 
    const isServiceStateLoading = (s: string) => (s===ServiceStateEnum.LOADING || s===ServiceStateEnum.INIT) ? true : false
    
    let isComboBoxLoading = () => isLoadingStateLoading() || isServiceStateLoading()
            ...
            {(item: NewUrchin) => (
                <Item key={item.id}>
                    <UrchinInfoContainer>
                        ...
                        <UrchinStatisticsRow> 
                            <UrchinStatistic>
                                <StarIcon />  {Math.round(item.frequency * 100).toPrecision(2)}
                            </UrchinStatistic>
                            <UrchinStatistic>  
