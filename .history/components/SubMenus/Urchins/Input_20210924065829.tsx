...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const onSelectionChange = (key: string) => {
        setFieldState((prevState) => {
            let selectedItem = prevList.items.find((option) => option.id === key);
            return {
                inputValue: selectedItem?.name ?? '',
                selectedKey: key, 
                items: prevState.items.filter((item) => startsWith(item.name, selectedItem?.name ?? '')
            )
        }
    })}
    ...
    const onInputChange = (value: string) => {
        setFieldState((prevState) => {
            return {
                ...
                items: prevState.items.filter((value) => startsWith(item.name, value))
            }}
        );
    }
    ...
    const handleInputChange = (value: string) => setFilterValue(event.currentTarget.value); 
