...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: string) => (ls===LoadingState.LOADING || ls===LoadingState.LOADING_MORE || ls===LoadingState.FILTERING || ls===LoadingState.SORTING) ? true : false; 
    ...
    let filteredItems: SavedUrchin[] = React.useMemo(() => {
        return list.items.filter((item: SavedUrchin) => contains(item.name, filterValue))
    }, [list, filterValue])
  
    let onSelectionChange = (key: string) => {
        let selectedItem: SavedUrchin = list.items.find((item: NewUrchin) => item.id===key)
        setSelectedKey(key)
