...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => { 
    ...
    const handlePersist = () => {
        ...
        // const { data, error } = useFetch<Post[]>(url, options)
    }

    const handleDelete = (itemKey: Key) => {
        let toDelete = list.getItem(itemKey) 
        let jsonHeaders = new Headers()
        jsonHeaders.append('Content-Type', 'application/json')

        let endpoint = `/api/urchins/user/sanshit.sagar@gmail.com/${label}/${toDelete.name}`
        let { delete, message }: { data: boolean; message: string;} = await fetch(endpoint, {
            method: 'DELETE',
            headers: jsonHeaders,
        })
        ...
        if(data) {
            ...
            toast.error(`Unable to delete ${toDelete.name}, Try Again?`)
        }
    }; 
