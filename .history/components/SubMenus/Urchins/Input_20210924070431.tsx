...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const onSelectionChange = (key: string) => {
        setFieldState((prevList) => {
            let selectedItem = prevList.items.find((option: SavedUrchin) => option.id === key)
            return {
                inputValue: selectedItem?.name ?? '',
                selectedKey: key, 
                items: prevState.items.filter((item: SavedUrchin) => startsWith(item.name, selectedItem?.name ?? ''))
            )
        }
    })}
