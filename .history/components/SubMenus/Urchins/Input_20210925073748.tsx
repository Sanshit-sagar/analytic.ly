...
const DeleteButton = (props: DeleteButtonProps) => {
    ...
    if(!loading && !children) return <Loading isIndeterminate type={'spinner'} />
    
    return (
        <Tooltip content={`Delete ${itemName}?`}>
            <Button size='xsmall' olor='transparent'  onClick={handleClick} css={{ fontSize: 12px}}
            >
                <DeleteButtonText>
                    {children}
                </DeleteButtonText>
...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {    
    ...
    let isLoading = React.useMemo(() => {
        ...
        if(state?.loadingState==='loadingMore') return true
        if(state?.loadingState==='sorting') return true
        if(state?.loadingState==='filtering') return true
        return false; 
    }, [state?.loadingState, selectedKey]);
    ...
    return (
        <ComboBox
            ...
            {...focusWithinProps}
        >
            {(item: NewUrchin, idx: number) => {
                if(isLoading) return <Item key={`waiting-for-item-${idx}-<Loading isIndeterminate={true/ value={inputValue}>
                return (
                    <Item key={item.id}>
                        <UrchinInfoContainer>
