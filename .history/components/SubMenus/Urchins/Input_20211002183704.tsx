...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => { 
    ...
    const handleDelete = (itemKey: Key) => {
        ...
        let { data, message }: DeleteApiResponse = await fetch(deleteEndpoint, {
            method: 'DELETE',
            headers: jsonHeaders,
        })
        ...
        if(data) {
            list.remove(itemKey)
            toast.success(`Successfully deleted ${toDelete.name}. ${message}`)
        } else {
            toast.error(`Unable to delete ${toDelete.name}, Try Again?.  ${message}`)
        }
    }; 
