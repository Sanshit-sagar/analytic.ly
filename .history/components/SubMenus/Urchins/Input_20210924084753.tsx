...
export const UtmParamInput = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const isLoadingStateLoading = (ls: string) =>  return (ls===LoadingState.LOADING || ls===LoadingState.LOADING_MORE || ls===LoadingState.FILTERING || ls===LoadingState.SORTING) ? true : false; 
        return fal
    };

    const isServiceStateLoading = (s: string) =>{
        return s==='loading' || s==='init' 
    };
