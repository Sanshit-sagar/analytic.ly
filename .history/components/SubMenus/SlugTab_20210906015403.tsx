...
const SaltTypePicker = () => {
    ...
    const updateSaltType = (value: sting, index: number) => {
        setSaltTypeIndex(value);
    }
    ...
    return (
        <ControlGroup>
            ...
            <ToolbarToggleGroup 
                ...
                onValueChange={(value: string) => updateSaltType(value, index)}
            > 
                {saltTypes.map((value: { id: number; icon: Element }, i: number) => {
                    return (
                        <ToolbarToggleItem 
                            key={`Salt-type-${i}`} 
                            value={value.id}
