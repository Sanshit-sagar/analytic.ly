...
const CategorisedResults = () => {
    const { data, loading, error } = useAsyncJotai('/api/urchins/user/sanshit.sagar@gmail.com')

    if(loading) return 'loading...'
    if(error) return 'error...'
    if(!data || !data?.userUrchins) return 'no data :('

    const categorizedUrchins = data.userUrchins
    const categories = Object.keys(categorizedUrchins)

    return (
        <Box css={{ height: '200px', width: '150px', display: 'flex', fd: 'row', jc: 'space-evenly', ai: 'stretch', gap: '$1' }}>
            {categories.map((category, i) => {
                return (
                    <Flex css={{ height: '100%', width: '100%', fd: 'column', jc: 'flex-start', ai: 'flex-start', gap: '$1' }}>
                        <Text size='1'>
                           {categorizedUrchins[category].length} matches
                        </Text> 
                        <br /> 
                        {categorizedUrchins?.length && categorizedUrchins[category]?.length &&
                            <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'center' }}>
                                    {categorizedUrchins[category].map((utmUrchin, i) => {
                                        return (
                                            <Text key={i} size='1' css={{ color: '$text' }}>  
                                                {utmUrchin} 
                                            </Text>
                                        )
                                    })}
...
const CategorizedUrchins = ({ urchins, userInput }: { urchins: any[]; userInput: string }) => {
    if(!urchins || urchins?.length === 0) return null;

    return (
        <Flex css={{ display: 'flex', fd: 'column', jc: 'flex-start', ai: 'center', gap: '$1' }}>
            {urchins.map((urchin) =>  (
                <Text css={{ color: '$accent' }}> {userInput} 
                    <Text css={{ color: '$text'}}> {urchin.substring(userInput.length)} </Text> 
                </Text> 
            ))}
        </Flex>
    )
...
const SeoParamsCacbe = () => {
    ...
    let isReady = !loading && !error && data
                ...
                {seoParams.map((seoParam: ISeoParameter, index: number) => (
                    <Flex css={{ fd: 'column', jc: 'center', ai: 'stretch', gap: '$1' }}>
                        <SeoParamsInput 
                            ...
                            index={index} 
                        />
                        <Text> {seoParam.id}: {isReady? `${categorizedUrchins[seoParam.id]?.length}` : '0'} matches </Text>
                        <CategorizedUrchins 
                            urchins={categorizedUrchins[seoParam.id].filter((urchin: string) => (
                                urchin.startsWith(seoParams[index].value)
                            ))} 
