...
export const ExpirationTabContent = () => {
    ...
    const [firstSelected, setFirstSelected] = useAtom<boolean>(firstSelectedAtom)
    const [selectedEndpoint, setSelectedEndpoint] = useAtom<'from | 'to' | 'null'>(selectedEndpointAtom)

    const handleDayClick = (day: Date) => {
        setSelectedEndpoint(firstSelected ? 'to' : 'from')
        if(!firstSelected) setFirstSelected(true)

        if(selectedEndpoint==='from') setRangeStart(day)
