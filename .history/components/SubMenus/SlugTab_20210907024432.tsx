...
const separators: ISeparator[] = [
    ...
    { textValue: '.', value: 'dot', alt: 'period', icon: <DotFilledIcon /> }
];
...
const SeperatorSelector = () => {
    ...
    const handleSelection = (selectedIndex: number) => {
        setSeparatorIndex(selectedIndex)
    }
    ...
    const SelectableText = ({ text, index }: { text: string; index: number }) => {
        return (
            <Text css={{ width: '100%', display: 'flex', fd: 'row', jc: 'space-between', ai: 'center' }}> 
                {text} ({separators[index].value})
            </Text>
        );
    }
                    ...
                    <SelectRadioGroup>
                        {separators.map((separator: ISeparator, index: number) => {
                            ...
                            return (
                                ...
                                >
                                    <SelectionIndicator index={index} />
                                    <SelectableText text={textValue} index={index} /> 
                                </SelectRadioItem>
                            )
                        })}
                    </SelectRadioGroup>
