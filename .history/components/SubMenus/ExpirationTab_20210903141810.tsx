...
const ErrorOrSelection = ({ errorStr, selectionStr }) => {
    return (<Text size='1' css={{ color: '$text' }}> {errorStr ? errorStr : selectionStr} </Text> 
}
...
const StartInput = () => {
    ...
    useEffect(() => {
        ...
        else if(selectedDay && !isDisabled) setSelectionStr(`${SELECTION_STR_PREFIX}: ${sd.toLocaleDateString()}`)
        else clearOutputs()
    ...
    return (
        <div style={{ width: '550px', backgroundColor: 'white' }}>
            ...
            <ErrorOrSelection errorStr={errorStr} selectionStr={selectionStr} />
