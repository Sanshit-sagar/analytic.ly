...
const StartInput = () => {
    ...
    const updateSelectionStr = (selectedDay: Date) => {
        setErrorStr(undefined)
        setSelectionStr(`${SELECTION_STR_PREFIX}: ${selectedDay.toLocaleDateString()}`)
    }
    const updateErrorStr = (errorStr: string) => {
    ...
    useEffect(() => {
        ...
        else if(selectedDay && !isDisabled) setSelectionStr(selectedDay);
            setErrorStr(undefined)
            setSelectionStr(`${SELECTION_STR_PREFIX}: ${selectedDay.toLocaleDateString()}`)
        }
        else {
