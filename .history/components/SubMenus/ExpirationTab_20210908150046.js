...
const showWeekNumbersAtom = atom()

const ExpirationTabContent = () => {
    const [month, setMonth] = useAtom(focussedMonthAtom)
    const [showWeekNumbers, setShowWeekNumbers] = useState(showWeekNumbersAtom)
    const [disableWeekends, setDisableWeekends] = useState(disableWeekendsAtom)
    const [showOutsideNumbers, setShowOutsideNumbers] = useState(showOutsideNumbersAtom)
