...
const isFirstSelected = atom(false)

export const ExpirationTabContent = () => {
    const [range, setRange] = useAtom(slugRangeAtom)

    const setRangeStart = useUpdateAtom(slugRangeStartAtom)
    const setRangeEnd = useUpdateAtom(slugRangeEndAtom)
    const [firstSelected, setFirstSelected] = useAtom(selectedEndpointAtom)
    const [selectedEndpoint, setSelectedEndpoint] = useAtom(selectedEndpointAtom)

    const handleDayClick = (day: Date) => {
        setSelectedEndpoint(firstSelected ? 'to' : 'from');
        if(!firstSelected) setFirstSelected()
        
        if(selectedEndpoint==='from') setRangeStart(new Date(day));
        else setRangeEnd(new Date(day));
    }
