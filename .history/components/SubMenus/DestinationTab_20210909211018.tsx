    ...
    return (
        <Text css={{ color: validationColor, mt: '$1', mb: '$1' }}>
            <Flex css={{ fd: 'row', jc: 'flex-end', ai: 'center', gap: '$2' }}>  
                <Text css={{ marginLeft: 'auto' }}>{isTyping ? 'typing' : 'not typing' }</Text> 
                <ValidationIcon />
                <> {validationResult} </>
            </Flex>
        </Text>
...
export const DestinationTabContent = () => {
    ...
    const [isTyping, setIsTyping  = useUpdateAtom(isTypingDestinationAtom)
    
    const [lastTypedAt, setLastTypedAt] = useState(undefined)
    let {keyboardProps} = useKeyboard({
        onKeyDown: (e: React.ChangeEventHandler<HTMLInputElement>) => {
            setLastTypedAt(new Date().getTime())
            setIsTyping(true)
        },
        onKeyUp: (e: React.ChangeEventHandler<HTMLInputElement>) => {
            setLastTypedAt(new Date().getTime())
        }
    });

    useEffect(() => {
        if(lastTypedAt - new Date().getTime() > 1000) {
            setLastTypedAt(false); 
        }
    }, [lastTypedAt]);
