...
export const ExpirationTabContent = () => {
    ...
    const [hoveredStartDate,setHoveredStartDate] = useState(undefined)
    const handleMouseMove = (event) => setHoveredStartDate(event.currentTarget.value)
    const handleMouseLeave = (event) => setHoveredStartDate(undefined)
    
    const handleClick = (event) => alert(event.currentTarget,id)
