...
const FmtDate = ({ date }) => {
    try {
        ...
        return <Text size='$4' css={{ color: '$funkyText'}}> {formatter.format(new Date(date))} </Text>
    } catch(error) {
...
//  const handleMouseMove = (event) => setHoveredStartDate(new Date(day).toLocaleString())
//  const handleMouseLeave = (event) => setHoveredStartDate(undefined)
//  const handleClick = (event) => alert(`You clicked ${new Date(day).toLocaleString()}`)
...
export const ExpirationTabContent = () => {
    ...
    return (
        ...
        <Flex css={{ height: '100%', width: '100%', fd: 'column', jc: 'space-around', ai: 'center', gap: '$4' }}>
            <FmtDate date={new Date(hoveredStartDate)} /> 
