...
const hoveredDateAtom = atom<Date | undefined>(undefined)
const activeMonth = atom<number>(new Date().getMonth())
const activeYear = atom<number>(new Date().getFullYear())
const rangeStartDateAtom = atom<Date | undefined>(undefined)
const rangeEndDateAtom = atom<Date | undefined>(undefined)

const selectionStageAtom = atom<number>(0)
const nextSelectionAssigneeAtomsAtom = atom<Atom<Date | undefined>>(rangeStartDateAtom)

const selectDateAtom = atom(
    null,
    (get, set, selectedDate) => {
        if(get(selectionStageAtom) === 2) return; 
     
        set(get(nextSelectionAssigneeAtomsAtom), selectedDate)

        let nextAtomForSelection = get(selectionStageAtom)===0 ? rangeStartDateAtom : get(selectionStageAtom)===1 ? rangeEndDateAtom : undefined
        let nextSelectionAssigneeIndex = get(selectionStageAtom) + 1  
        set(nextSelectionAssigneeAtomsAtom,nextAtomForSelection)
        set(selectionStageAtom, nextSelectionAssigneeIndex)
    }
)

const updateHoveredDateAtom = (
    null, 
    (get, set, hoveredDate: Date) => {
