...
const NavPrevButton = ({ className,onPreviousClick }: { className: string; onClick: () => void; transform: (n: number) => number; }) => {
    const [activePage, setActivePage] = useAtom(activePageAtom)
    
    const navigateToPrevMonth = () => {
        let updatedMonth = new Date(activePage).getMonth() - 1
        let updatedYear = updatedMonth===12 ? transform(getYear(activePage) - 1 : getYear(activePage) 
