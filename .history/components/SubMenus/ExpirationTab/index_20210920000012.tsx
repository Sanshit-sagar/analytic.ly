...
const sanitizeDate = (d: Date | number | string | undefined | null) => {
    return !d || typeof d !== Date ? new Date().getTime() : new Date(d).getTime()
}
...
const ToDate = () => {
    ...
    return <FmtDate date={new Date(enteredTo)} fallback={new Date(to)} /> 
}

const FromDate = () => {
    const [from, setFrom] = useState<Date | undefined | null>(null)    
    const [hoveredStartDate, setHoveredStartDate] = useAtom(hoveredStartDateAtom)
    return <FmtDate date={new Date(hoveredStartDate || new Date().getTime())} fallback={new Date(sanitizeDate(from))}/> 
}
