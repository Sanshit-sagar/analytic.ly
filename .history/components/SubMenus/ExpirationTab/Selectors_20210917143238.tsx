...
function getMonthsInRange(localeUtils:): ICalendarItem[] {
...
const YearSelector = ({ onChange, fromMonth, toMonth }: IYearSelectorProps) => {
    ...
    const handleYearChange = (updatedYear: number, updatedMonth: number) => {
        setYear(updatedYear);
    ...
    return (
        <SelectMenu
            ...
            items={yearItems}
            selectedIndex={year}
            setSelectedIndex={(value: number) => handleYearChange(parseInt(`${value}`) + 2021, month)}
            selectedValue={`${year}`}
