...
export const FmtDate = ({ formattedDate, type }: { formattedDate: string, type: string; }) => {
    return <FormattedDateText> {type}: {formattedDate} </FormattedDateText> 
}

const sanitizeDate = (d: any | undefined | null): string => {
    let formatter = useDateFormatter();
    try {
        let sd = !d ? new Date() : new Date(d)
...
const ToDateInput = () => {
    ...
    return <FmtDate date={sanitizeDate(enteredTo)}  type="START"/> 
}

const FromDateInput = () => { 
    const hoveredStartDate = useAtomValue(hoveredStartDateAtom)
    return <FmtDate date={sanitizeDate(hoveredStartDate)}  type="END"/> 
}
