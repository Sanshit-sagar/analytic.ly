...
const SeoTabContent = () => {
    ...
    const seoParams = [
        ...
        { id: 'campaign', value: campaign,  setter: (event) => setTerm(event.currentTarget.value)},
        { id: 'content', value: content,  setter: (event) => setTerm(event.currentTarget.value)},
    ];
            ...
            {seoParams.map((seoParam, i) => {
                ...
                <TextField
                    ...
                    onChange={(event: React.ChangeEventHandler<HTMLInputElement>) => (
                        handleSeoTagUpdate(event, seoParam.id]);
                    )}
