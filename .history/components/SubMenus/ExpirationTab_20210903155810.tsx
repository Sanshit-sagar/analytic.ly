...
const StartInput = ({ key }: { key: string }) => {
    ...
    const handleDayChange = (updatedDay: Date, modifiers: DayModifiers, dayPickerInput: DayPickerInput) => {
        ...
        if(!isEmpty && !isDisabled && updatedDay) {
            ...
            messageStr = `{SELECTION_STR_PREFIX}: ${updatedDay.toLocaleDateString()} !! woow` 
        } else {
...
export const ExpirationTabContent = () => (
    <Flex css={{ fd: 'row', jc: 'flex-start', ai: 'center', gap: '$2', width: '100px' }}>
         <StartInput key={'start'} />
         <DateInput 
