...
const updateGroupCountAtom = atom(
    null, 
    (get, set, updatedCount: number) => {
        set(abTestNumGroupsAtom, updatedCount);
        let stepSize = (maxValue - minValue) / groupCount;
        
        let temp: number[] = [...Array(updatedCount)]; 
        temp[0] = 0
        for(let i = 1; i<=updatedCount; i++) temp[i] = Math.round((temp[i-1] + stepSize) * 100)/100
        return temp
