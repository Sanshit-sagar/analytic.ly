...
const ConfigTree = () => {
    const abTestConfigs = useAtomValue(abTestGroupsAtom)
    const updateUrlAtIndex = useUpdateAtom(setUrlAtIndexInTestGroup)

    return (
        <CustomTree 
            slug={'the-best-slug-5-evah'} 
            abTestConfigs={abTestConfigs} 
            updateUrlAtIndex={(event: React.ChangeEvent<HTMLInputElement>, index: number) => updateUrlAtIndex({ url: event.currentTarget.value, index})}
        />
    )
}
...
export const AbTestingTab = () => {
    ...
    return (
        <AbTestingContainer>
            <Controller />
            <AbPercentages /> 
            <p> {JSON.stringify(abTestConfigs)} </p> 
            <ConfigTree /> 
        </AbTestingContainer>
    );
}
