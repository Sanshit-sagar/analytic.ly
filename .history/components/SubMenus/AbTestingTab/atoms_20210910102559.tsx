...
const filteredAtom = atom<string[]>((get) => {
    ...
    else return alternates.filter((id) => !get(alternateAtomFamily({ id })).active)
}); 
...
const AlternateItem:React.FC<IAlternateItemProps> = ({ id, remove }) => {
    ...
    const [item, setItem] = useAtom(alternateAtomFamily({ id }));
...
export const AlternatesList = () => {
    ...
    const remove = (id: string) => {
        setAlternates((prev: string[]) => prev.filter((item: string) => item !== id));
    ...
    const add = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const id = nanoid()
        alternateAtomFamily({ id, title })
        setAlternates((prev: string[]) => [...prev, id])
    }
