...
export const AbTestConfigsTree = () => {
    const [abTestConfigAtoms, removeAbTestConfigAtom] = useAtom(abTestConfigAtomsAtom)
    const slug = 'lollipop-fifty-10110'

    return (
        <TreeContainer>
            <Tree name={slug} defaultOpen>
                {abTestConfigAtoms.map((abTestConfigAtom, index: number) => (
                    <Flex css={{ fd: 'row', jc: 'flex-start', ai: 'center', gap: '$2', width: '100%' }}>
                        <AbTestConfig
                            abTestConfigAtom={abTestConfigAtom}
                            remove={() => removeAbTestConfigsAtom(abTestConfigAtom)}
                        />
