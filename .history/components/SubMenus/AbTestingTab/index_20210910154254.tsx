...
const Controller = () => {
    const [alternateCount, setAlternateCount] = React.useState(4)
    const [updatedVal, setUpdatedVal] = React.useState(0)

    let [partitionSize, setPartitionSize] = React.useState(Math.round(100/alternateCount))
    let [values, setValues] = React.useState([...Array(alternateCount)].map((_, i) => i*partitionSize + 1))
    ...
    React.useEffect(() => {
        if(alternateCount < values.length) {
            ...
            setValues([
                ...values, 
                ...Array(alternateCount - values.length).map((_, i) => values[i-2] + partitionSize + 1)
            ]);
        }
    ...
    return (
        <Toolbar>
            ...
            />
            <ToolbarSeparator />
            <Text size='3'> LAST UPDATED: {JSON.stringify(values)} </Text>
        </Toolbar>
    );
}
