...
const handleInitValues = ({ alternateCount, stepSize, reassignValues }: IInitValsProps): void => {
...
const Controller = () => {
    ...
    const [stepSize, setStepSize] = React.useState<number>(Math.round(((maxValue - minValue)*100)/(alternateCount))/100)
    
    const [values, setValues] = React.useState<number[] | undefined | null>([])
    const reassignValues = (temp: number[]) => setValues([]
    ...
    React.useEffect(() => {
        ...
        setStepSize(Math.round(((maxValue - minValue)*100)/(alternateCount))/100)
    ...
    }, [alternateCount, setMaxValue, setMinValue]);
