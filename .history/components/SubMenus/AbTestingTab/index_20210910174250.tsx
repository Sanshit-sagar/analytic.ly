...
const handleInitValues = ({ alternateCount, stepSize }: IInitValsProps): number[] => {
...
const Controller = () => {
    ...
    let stepSize = Math.round(range/alternateCount)/100
    ...
    const handleAlternateCountChange = (updatedAltCount: number) => {
        ...
        stepSize = Math.round(range/alternateCount)/100;
        setValues([...handleInitValues({ alternateCount, stepSize })]);
    }

    return (
        <Toolbar>
            <Text> - {maxValue} - {minValue} </Text>
            <NumberField
                label='Alternates'
