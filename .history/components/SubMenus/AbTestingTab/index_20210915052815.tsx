...
const AbPercentages = () => {
    const abTestPercentages = useUpdateAtom(abTestGroupDiversionPercentAtom)

    return (
        <Flex css={{ fd: 'row', jc: 'space-evenly', ai: 'center', gap: '$3' }}>
            {abTestPercentages.map((abpct: numbe))}
            <Text size='3' css={{ color: '$funkyText', '&:hover': { color: '$funky' }}}>
                {}
            </Text>
        </Flex>
    )
