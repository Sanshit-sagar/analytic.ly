...
const ONE_HUNDRED_PERCENT = 1.00
...
const handleInitValues = (alternateCount: number, stepSize: number): number[] => {
    ...
    for(let i = 1; i<=alternateCount; i++) {
        temp[i] = (i===0) ? 0 : Math.round((temp[i-1]+stepSize)*100)/100
    }
    return temp
}
...
const Controller = () => {
    ...
    const [minValue, _] = React.useState<number>(ONE_PERCENT)
    const [maxValue, _] = React.useState<number>(ONE_HUNDRED_PERCENT)
