...
const ExpirationTabContent = () => {
    ...
    function getRRangeInMs(range) {
        let intervalStart = new Date(range[0])
        let intervalEnd = new Date(range[1])
    ...
    const numSelectedMs = selectedDays && selectedDays?.length===2 ? getRangeInMs(selectedDays) : 0
    const numSelectedDays = numSelectedMs / NUM_MSP_PER_DAY
