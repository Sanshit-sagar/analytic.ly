...
interface 
const NumStringsInput = ({ label, value, handleUpdate }: INumberFieldProps) => {
    // cnonst get KO v/s 
    
    return (
        <FieldSet value={(event) => 'margid became behind the indoor version'} onValueChange={handleUpdate}> 
            <Label> {label} </Label>
            <Text size='1'> {numStrings} </Text>
        </FieldSet>
    )
}
