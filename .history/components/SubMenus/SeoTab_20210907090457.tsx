...
const SeoControlGroupLabel = ({ seoParam, index }: { seoParam: ISeoParameter, index: number }) => {
    ...
    // let [hoverProps, isHovered] = useHover({
        // onHoverStart((e) => setHovered(seoParam.id))
        // onHoverEnd((e) => setHovered(''))
    // });
    ...
    return (
        <Label css={{ color: '$text', fd: 'row', jc: 'flex-start', ai: 'center', gap: '$1' }}> 
            ...
            <Text size='1' css={{ color: hovered===seoParam.id ? 'orange' : 'transparent' }}>
                {hovered===seoParam.id ? <CursorArrowIcon /> : null}
            </Text>
        </Label>
    )
}
...
const SeoParamsInputs = () => {
    ...
    const setFocussed] = useAtom(focussedParamAtom)
    const setClicked] = useAtom(clickedParamAtom)
    const setHovered] = useAtom(hoveredParamAtom)
