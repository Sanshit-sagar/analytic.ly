...
const EventIconsGroup = ({ seoGroup, seoParam }: { seoGroup: SeoGroupEventConfig; seoParam: ISeoParameter }) => {
    ...
    let isItemCulprit = eventCulprit===seoParam.id
    ...
    return (
        <Label>
            <FlexStartCenterRow>
                ...
                <FlexEndCenterRow>
                    {Object.entries(seoGroups).map((seoGroupConfigEntry: [string, SeoGroupEventConfig], configIndex: number) => (
                        <EventIconsGroup 
                            key={`${group}-${index}-item-{configIndex}`}
