...
const PanelWrapper = ({ submenu }: { submenu: ITabItem }) => (
    ...
    <Flex css={{ height: '100%', width: '1000px', fd: 'column', jc: 'flex-start', ai: 'stretch', padding: '$1' }}>
        <Flex css={{ height: '50px', fd: 'row', jc: 'flex-start', ai: 'flex-end', gap: '$2'}}>
            <Heading size='2'>
                {submenu.icon}
                {submenu.value.toUpperCase()} 
            </Heading>

            <Text size='2' css={{ color: '$text', fontSize:15, lineHeight: '20px', textTransform: 'uppercase'}}>
                {descriptions[parseInt(submenu.id)].toUpperCase()}
            </Text>
        </Flex>
