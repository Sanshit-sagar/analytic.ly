...
function mapExistsAndMapHasKey(map: {[key: string]: string })  {
    return categorizedUrchins && categorizedUrchins[seoParam.id] && categorizedUrchins[seoParam.id].length 
}
        ...
        <FlexCenterCenterColumn>
            <FlexCenterCenterRow>
                {seoParams.map((seoParam: ISeoParameter, index: number) => {
                    let urchinsForCategory = mapExistsAndMapHasKey(categorizedUrchins, seoParam.id) ? categorizedUrchins[seoParam.id] : []
