...
const selectedSlugCategoryValueAtom = atom<any>((get) => slugCategoryOptions[get(selectedSlugCategoryIndexAtom)].category);


export const SlugCategorySelector = () => {
    const [isOpen, setIsOpen] = useAtom(isSlugCategoryActive)
    const [selectedIndex, setSelectedIndex] = useAtom(selectedSlugCategoryIndexAtom)
    const [selectedCategory, setSelectedCategory] = useAtom(selectedSlugCategoryValueAtom)
                    ...
                    <SelectRadioGroup>
                        {slugCategoryOptions.map((option: ISlugCateogry, index: number) => {
                            return (
                                ...
                                >
                                    <Text size='1'>{index} : {option.category}</Text> 
                                </SelectRadioItem>
                            );
                        })}
                    </SelectRadioGroup>
...
export const SlugSelectionMenu = () => {
    ...
    return (
        <Text size='1'> 
            {isOpen.toString()} | {selectedIdx} | {selectedVal} 
        </Text>
    )
}
