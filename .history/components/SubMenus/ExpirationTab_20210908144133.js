...
const ExpirationTabContent = () => {
    ...
    let intervalStar, intervalEnd;
    
    function getRangeInMs(range) {
        try {
            intervalStart = new Date(parseInt(range[0])).getTime()
            intervalEnd = new Date(parseInt(range[1])).getTime()
    ...
    let numSelectedDays = numSelectedMs / NUM_MSP_PER_DAY
