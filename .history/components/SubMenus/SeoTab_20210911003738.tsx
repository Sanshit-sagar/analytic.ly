...
const categorizedUrchinsAtom = atom<string[]>([])
const isReadyAtom = atom<boolean>(false)
const isResponseFetchedAtom = atom<boolean>(
    (get) => get(isReadyAtom),
    (_get, set, update) => set(isReadyAtom, (!get(loadingAtom) && !get(errorAtom) && get(data)) ? true : false),
);

const filterUrchinsAtom = (
    null,
    (_get, set, update) => set(categorizedUrchinsAtom, get(isResponseFetchedAtom) ? get(responseDataAtom()))
)
