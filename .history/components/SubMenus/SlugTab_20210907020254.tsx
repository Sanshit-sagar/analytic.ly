...
const separators: ISeparator[] = [
    { symbol: '-', value: 'hyphen', alt: 'dash', icon: <DashIcon /> },
    { symbol: '*', value: 'star', alt: 'asterisk', icon: <StarIcon /> }, 
    { symbol: '_', value: 'underscore', alt: undefined, icon: undefined },
    { symbol: '#', value: 'hashtag', alt: 'pound', icon: undefined  },
    { symbol: ':', value: 'colon', alt: undefined, icon: undefined },
    { symbol: '/', value: 'slash', alt: 'divide', icon: <SlashIcon />  },
    { symbol: '~', value: 'tilde', alt: 'twiddle', icon: undefined },
    { symbol: '+', value: 'plus', alt: 'add', icon: <PlusIcon /> },
    { symbol: '--', value: 'minus', alt: undefined, icon: <MinusIcon /> },
];
...
export const separatorAtom = atom((get) => separators[get(separatorIndexAtom)].symbol)
...
type StitchesComponent =  React.ChangeEvent<StitchesComponentWithAutoCompleteForReactComponents<ExtendedPrimitive>>
type RadioChangeEvent = StitchesComponent<ExtendedPrimitive<MenuRadioItemPrimitive, "div">>

const SeperatorSelector = () => {
    const [isOpen, setIsOpen] = useState(false)
    const [separatorIndex, setSeparatorIndex] = useAtom(separatorIndexAtom)

    const handleSelection = (event: RadioChangeEvent) => {
        setSeparatorIndex(event.currentTarget.value)
    }
...
const SlugTabContent = () => {
    ...
    return (
        <Flex css={{ width: '50%', fd: 'column', jc: 'flex-start', ai: 'center', gap: '$1'}}>
