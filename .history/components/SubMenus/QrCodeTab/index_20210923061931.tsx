...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    const handlePersist = () => {
        const newItem = createNewUrchin(label, value, list.items?.length || 0)
        list.append(newItem)
        addUrchinToUserCollection(label, newItem)
        list.reload()
    }
    ...
    return (
        <ComboBox
            ...
            isLoading={
                list.loadingState.loading || list.loadingState.loadingMore || list.loadingState.filtering || list.loadingState.sorting || serviceState.loading || serviceState.init
            }
