...
export const UrchinLists = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    const [status, setStatus] = useState<LoadingStatus>(LoadingStatusEnum.IDLE)
    ...
    const handlePersist = () => {
        ...
        setStatus(LoadingStatusEnum.LOADING_MORE)

        publishNewUrchin(label, newItem)
            .then((response) => setStatus(LoadingStatusEnum.LOADED))
            .catch((error) => setStatus(LoadingStatusEnum.ERROR))
    }
