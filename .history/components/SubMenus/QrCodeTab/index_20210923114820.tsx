...
export const UrchinLists = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    return (
        <>
            ...
            {service.status==='loading' &&  <h3> loading... </h3>}
            {service.status==='loaded' && <h3> loaded! </h3>}
            {service.status==='loaded' && response && JSON.stringify(response)}
            {service.status==='error' || error && <h3> error </h3>}
        </>
    )
}
