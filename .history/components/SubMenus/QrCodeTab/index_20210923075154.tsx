...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    useEffect(() => {
        setFieldState((prev: IFieldState) => { ...prev, items: !list?.items?.length ? [] : [...list.items] })
    }, [list])
    ...
    let onInputChange = (value: Key) => {
        setFieldState((prevState: IFieldState) => {
            setValue(value)
            let matchedUrchins =  list.items.filter((listItem) => contains(listItem.name, value))

            return {
                ...prevState,
                inputValue: value,
                selectedKey: matchedUrchins?.length ? matchedUrchins[0] : prevState.selectedKey,
                items: [...list.items.filter((listItem) => contains(listItem.name, value))]
            }
        })
    }
    ...
    return (
        <ComboBox
            key={key}
            label={label}
            items={list.items.filter((listItem) => listItem.contains())}
