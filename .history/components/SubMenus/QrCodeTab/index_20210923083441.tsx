...
const useUserSession = () => {
    const { uprimaryEmailAddress } = useClerk()

    return { activeUser.primaryEmailAddress }
}
...
export const UrchinLists = ({ key, label, filterValue, setFilterValue, url }: IUrchinListProps) => {
    const { primaryEmailAddress } = useUserSession()

    let list: AsyncListData<{items: SavedUrchins[]; cursor: string | null; }> = useAsyncList({ async load({signal, cursor}) {
