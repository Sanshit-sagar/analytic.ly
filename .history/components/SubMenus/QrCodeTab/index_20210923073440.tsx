...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    let onInputChange = (value: Key) => {
        setFieldState((prevState: IFieldState) => {
            updateValue(value)
            let matchedUrchins = items.find((item) => startsWith(item.name, value));

            return {
                ...prevState,
                inputValue: value,
                selectedKey: matchedUrchins?.length ? items?.length ? items[0] : prevState.selectedKey : prevState.selectedKey;
                }
