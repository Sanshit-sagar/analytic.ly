...
const useUserSession = () => {
    ...
    const { data, username, firstName, lastName, fullName, primaryEmailAddressId, primaryEmailAddress, primaryPhoneNumberId, primaryPhoneNumber, profileImageUrl, data, publicMetadata, unsafeMetadata }
    data","username","emailAddresses","phoneNumbers","externalAccounts","passwordEnabled","firstName","lastName","fullName","primaryEmailAddressId","primaryEmailAddress","primaryPhoneNumberId","primaryPhoneNumber","profileImageUrl","publicMetadata","unsafeMetadata","updatedAt","createdAt","cachedSessionsWithActivities","isPrimaryIdentification","twoFactorEnabled","createEmailAddress","createPhoneNumber","update","delete","getToken","getSessions","setProfileImage"]
...
export const UrchinLists = () => {
    ...
    return (
        <>
        <Text size='2' css={{ color: '$text', maxWidth: '500px', display: 'flex', fd: 'column', flexWrap: 'wrap', jc: 'flex-start', ai: 'flex-start' }}> 
            HIHIHI {JSON.stringify(Object.keys(activeUser))}
        </Text> 
        <ComboBox
            label={'Async list'}
            items={list.items}
