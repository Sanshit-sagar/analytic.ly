...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    let filteredItems = useMemo(() => list.items.filter((item) => startsWith(item.email, filterValue)),
    [options, filterValue]);
    ...
    let onInputChange = (value: Key) => {
        setFieldState({
            let matchedUrchins: AsyncListData<SavedUrchin> = list?.items?.filter((item) => startsWith(item.name, value))
            setValue(value)

            return {
                inputValue: value,
                selectedKey: matchedUrchins?.length ? matchedUrchins[0] : prevState.selectedKey,
                items: list.items.filter((item) => startsWith(item.name, value))
            }
        });
    }
