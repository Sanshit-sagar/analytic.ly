...
function createNewUrchin(category: string, initValue: string, listIndex: number) {
    return {
        id: `${listIndex}`,
        category,
        name: initValue,
...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    const addUrchinToUserCollection = (key: UrchinCategoryType, value: U) => {
    ...
    const handlePersist = () => {
        const newItem = createNewUrchin(key, value, list.items?.length || 0)
        list.append(newItem)
