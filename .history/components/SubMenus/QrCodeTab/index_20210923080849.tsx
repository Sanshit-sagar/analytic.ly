...
export const UrchinLists = ({ key, label, value, setValue, url }: IUrchinListProps) => {
    ...
    let onInputChange = (value: Key) => {
        setFieldState({
            setValue(value)
            let matchedUrchins: SavedUrchin[] =  list.items.filter((listItem: SavedUrchin) => startsWith(listItem.name, value))

            return {
                inputValue: value,
                selectedKey: matchedUrchins?.length ? matchedUrchins[0] : prevState.selectedKey,
                items: [...list.items.filter((listItem: SavedUrchin) => startsWith(listItem.name, value))]
            }
        })
    }
