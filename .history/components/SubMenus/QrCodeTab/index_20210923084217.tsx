...
export const UrchinLists = ({ key, label, filterValue, setFilterValue, endpoint }: IUrchinListProps) => {
    ...
    let list: AsyncListData<{items: SavedUrchin[]; cursor: string | null; }> = useAsyncList({ async load({signal, cursor}) {
    ...
    let filteredItems: SavedUrchin[] = useMemo(() => {list.items.filter((item: SavedUrchin) => startsWith(item.name, filterValue));
