...
const StartInput = () => {
    ...
    const handleDayChange = (updatedDay: Date, modifiers: DayModifiers, dayPickerInput: DayPickerInput) => {
        ...
        const mssg = isEmpty ? NO_INPUT_ERROR :  !selectedDay ?  INVALID_DAY_ERROR : isDisabled ? DISABLED_DAY_ERROR : undefined
        if(selectedDay && mssg===undefined) {
            mssg = {SELECTION_STR_PREFIX}: ${selectedDay.toLocaleDateString()}` 
        }
        ...
        dispatch({ 
            ...
            payload: { 
                key: 'start', 
                value: updatedDay, 
                message: isEmpty ? NO_INPUT_ERROR : !selectedDay ? INVALID_DAY_ERROR : isDisabled ? DISABLED_DAY_ERROR : `$
            }
        })
