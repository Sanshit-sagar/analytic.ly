...
import { UrlFieldSet, Label, Input } from '../../primitives/UrlFieldSet' 
...
const getSuggestedSlugs = ({ saltLength, numStrings, separator, saltType }: ISuggestedSlugsProps) => {
    let isManly = false; 
    ...
    return (
        <UrlFieldSet> 
            ...
            <Input 
                key={`label-${label}`}
                value={value} 
                onValueChange={handleUpdate}dqqq
            />
        </UrlFieldSet>
    )
}
...
const SlugTabContent = () => {
    ...
    const { suggestions, loading, error } = getSuggestedSlugs({ saltLength, numStrings, separator, saltTypeIndex })    
    ...
    return (
        <>
             <UrlFieldSet css={{ width: '100%', fd: 'column', jc: 'flex-start', ai: 'stretch'}}>
                ...
                <Text>
                    {loading ? 'loading...' : error ? `${error.message}` : JSON.stringify(suggestions)}
                </Text>
