...
function sanitizedResultsLine(res: string): string {
    return res.length > 75 ? '' : res;
}
        ...
        <ApiResponseContainer>
            <Tree name={hybridGraph[hybridGraphKeys[0]]} defaultOpen>                
                {hybridGraphKeys?.filter((k: string) => hybridGraph[k]!==null).map((key: string, idx: number) => {
                    let sanitizedValue = sanitizedResultsLine(hybridGraph[key])
                    let requiresScrollArea = sanitizedValue.length 
                    return (
                        <Tree 
                            key={idx} 
                            name={`${key}:${sanitizedValue}`} 
                            defaultOpen
                        >
                            {sanitizedValue} <Text>{hybridGraph[key]}</Text>
                        </Tree>
                    );
                })}
            </Tree>
