...
export const separatorAtom = atom((get) => separators[parseInt(get(separatorIndexAtom))].textValue)
export const saltTypeAtom = atom((get) => saltTypes[parseInt(get(saltTypeIndexAtom))].id)
...
const SeperatorSelector = () => {
    ...
    const SelectTriggerWithIcon = ({ index }: { index: number }) => {
        const selectableText = isValidIndex(index, separators) ? separators[index]?.textValue : ''
        return (
            <SelectTrigger>
                <Text> {selectableText} <ChevronDownIcon /> </Text>
            </SelectTrigger>
        );
    }
