...
const yearAtom = atom(
    (get) => new Date(get(monthAtom)).getFullYear(),
)

const MonthSelector = ({ onChange, localeUtils }) => {
    const [month, setMonth] = useAtom(monthAtom)
    const [year, setYear] = useAtomValue(yearAtom);

    const handleMonthChange = (updatedMonth,updatedYear) => {
        setMonth(updatedMonth);

        alert(`Before update: month=${updatedMonth} and year=${year}`)
    ...
    return (
        <SelectMenu 
            ...
            setSelectedIndex={(value) => handleMonthChange(value, new Date(value).getFullYear() )}
...
const YearSelector = ({ onChange }) => {
    ...
    for (let i = fromMonth.getFullYear(); i <= 3; i += 1) { 
        yearItems.push({
            value: `${i}`,
