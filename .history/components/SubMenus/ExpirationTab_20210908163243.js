...
const rangeAtom = atom({ from: undefined, to: undefined, enteredTo: undefined})
const fromAtom = focusAtom(rangeAtom, (optic) => optic.prop('from'))
const toAtom =  focusAtom(rangeAtom, (optic) => optic.prop('to'))
const enteredToAtom = focusAtom(rangeAtom, (optic) => optic.prop('enteredTo'))

const rangeAtomsAtom = (
    (get) => get(rangeAtom),
    (_get, set, update) => set(rangeAtom, {...update})
);
const fromAtomsAtom = (
    (get) => get(fromAtom),
    (_get, set, update) => set(fromAtom, update) 
)
const toAtomsAtom = (
    (get) => get(toAtom),
    (_get, set, update) => set(toAtom, update)
)
const enteredToAtomsAtom = (
    (get) => get(enteredToAtom),
    (_get, set, update) =>set(enteredToAtom, update)
)
const isDayBeforeAtom = (
    (_get, set, day) => DateUtils.isDayBefore(day, get(fromAtomsAtom))
)
const isRangeSelectedAtom = (
    (get) => get(fromAtomsAtom) && get(toAtomsAtom),
)
const isSelectingFirstDay = (
    (get) => !get(fromAtomsAtom) || get(isDayBeforeAtom) || get(isRangeSelectedAtom)
)
...
const Options = () => {
    ...
    const options = [
        { id: 'weeks', value: showWeeks, handleToggle: () => setShowWeeks(!showWeeks) },
        { id: 'outsides', value: showOutsides, handleToggle: () => setShowOutsides(!showOutsides) },
        { id: 'weekends', value: disabledWeekends, handleToggle: () => setDisabledWeekends(!disabledWeekends) },
    ]; 
    ...
    return (
        <>{options.map((option, index) => (
            <button 
                ...
                style={{ padding: '5px', border: 'thin solid black', color: 'black' }}
            >
...
const ExpirationTabContent = () => {
    ...
    const [enteredTo, setEnteredTo] = useAtom(enteredToAtom)
