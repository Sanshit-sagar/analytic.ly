...
import { StarIcon, GearIcon, ChevronLeftIcon, ChevronRightIcon } from '@radix-ui/react-icons'
...
function Navbar({
    ...
    return (
        <>
            <ToggleButton css={{ float: 'left' }} onClick={() => onPreviousClick()}>
              {/* ← {prev.slice(0, 3)} */}
              <ChevronLeftIcon />
            </ToggleButton>
            <ToggleButton css={{ float: 'right' }}  onClick={() => onNextClick()}>
                {/* {next.slice(0, 3)} → */}
            </ToggleButton>
        </>
    );
    ...
    return (
        <StyledCalendarContainer>
            <DayPicker 
                ...
                showOutsideDays={showOutsideNumbers}
                month={month}
                fromMonth={from}
                modifiers={modifiers}
