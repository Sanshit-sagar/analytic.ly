...
const BarChart = ({  
    ...
    function formatTooltipProps(d: any, i: number) {
        ...
        const viewCount: string = isFreq ? `Total: ${freq(d)}` : `Unique: ${unique(d)}`
        const slugName: string = slug(d).substring(0,25)

        let tooltipData = {
            slugAndViews: `${slugName} ${viewCount}`,
        ...
        <Box css={{ mr: '$3', width: '95%' }}>
            ...
            >
                {freqsBg.map((d: Datum, i: number) => {
                    ...
                    const fill = i%2===0 ? colors.funky : darkMode ? colors.hiContrast : colors.loContrast
