...
const BarChart = ({  
    ...
    function formatTooltipProps(d: any, i: number) {
        ...
        const viewCount: string = isFreq ? `${freq(d)} TOTAL CLICKS` : `${unique(d)} UNIQUE CLICKS`
        const slugName: string = slug(d).substring(0,25)
        const normalizedScore: number = isFreq ? `${Math.round(normal(d)*100)}%` : uniqueNormal(d)
        const slugRank: isFreq ? rank(d) : uniqueRank(i)
                        ...
                        {Object.entries(tooltipData).map((d: [string, string | number | string[]], i: number) => {
                            return (
                             <Flex key={i} css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'flex-start' }}> 
                                 <Text css={{ color: colors.text }}> {tooltipLabels[`${d[0]}`]} </Text>
                                 <Text css={{ color: colors.text }}> {d[1]} </Text>
                             </Flex>
                            );
                        })}
                    </TooltipWrapper>
