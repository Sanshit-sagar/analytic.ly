...
const BarChart = ({  
    ...
    function formatTooltipProps(d: any, i: number) {
        ...
        let tooltipData = {
            slug: slugName,
            rank: slugRank,
            slugAndViews: `${viewCount} (${normalizedScore})`
        };
        return tooltipData; 
    }
                        ...
                        {Object.entries(tooltipData).map((d: [string, string | number | string[]], i: number) => {
                            return (
                             <Flex key={i} css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'flex-start' }}> 
                                 {i===0 ?
                                 <Text css={{ color: colors.funky, textDecoration: 'underline', textDecorationColor: '$funky', fontWeight: '400', mb: '$1' }}>  {d[1]} </Text> :
                                 {tooltipLabels[`${d[0]}`]} </Text>
