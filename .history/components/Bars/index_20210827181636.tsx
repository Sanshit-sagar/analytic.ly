        ...
        <div style={{ position: "relative" }}>
            ...
            {tooltipOpen && tooltipData && (
                <TooltipInPortal
                    ...
                    style={tooltipStyles}
                >  
                    <Box css={{ bc: '$canvas', width: '225px', height: '150px', br: '$1', padding: '$1' }}>
...
const GroupedBars = () => {
    ...
    let freqs: Datum[] = views.frequency?.length ? views.frequency.sort((a: Datum,b: Datum)=>b.score-a.score) : []
    
    useEffect(() => {
        if(freqs.length && !page.length)
    })

    const getNextCols = () => {
        setIndex(page + PAGE_SIZE)
        setPage([...views.frequency.sort((a: Datum, b: Datum) => (b.score - a.score)).slice(index, Math.min(views.frequency.length, index + PAGE_SIZE))])
    }
