  ...
  return (
      <svg width={width} height={height}>
        ...
        <AnimatedAxis 
          orientation="left" 
          numTicks={4}
          tickValues={clicks.map((click: any, _:number) => click.y).filter((y: number, _: number) => Math.max(y, 1/y) === y)}
        /> 
        <AnimatedAxis 
          orientation="bottom" 
          numTicks={2}  
          tickValues={clicks.map((click: any, i: number) => click[i].x).filter((cx: any, _: number) => cx%(clicks.length/4)===0)}
