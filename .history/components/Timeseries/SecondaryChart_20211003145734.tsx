...
import { recalcDataAtom, activeFilteredDataAtom, filteredDataAtom, boundsAtom } from '../../atoms/timeseries'
...
const SecondaryChart: React.FC<SecondaryChartProps> = ({
    ...
    let { start, end }:{ start: number; end: number; } = { 
        start: minTimestamp || new Date(new Date().getFullYear(), 1, 1).getTime(), 
    ...
    const activeFilteredData = useAtomValue(activeFilteredDataAtom)
    ...
    useEffect(() => {
        if (data.length) {
            ...
            setBounds({ 
                x0: getDate(activeFilteredData[0]).getTime(), 
                y0: getClickScore(activeFilteredData[0]), 
                x1: getDate(activeFilteredData[activeFilteredData.length-1]).getTime(), 
                y1: getClickScore(activeFilteredData[activeFilteredData.length-1]),
            })
        }
    }, [activeFilteredData, updateFilteredData])
        ...
        <div style={{ position: "relative" }}>
            <svg width={width} height={height}>
                ...
                <AreaChart
                     ...
                     xScale={dateScale}
                     yScale={clickScale}
                     gradientColor={colors.funkyText}
                >
                    <PatternLines
                        id={PATTERN_ID}
                        height={7}
