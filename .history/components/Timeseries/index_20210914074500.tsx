...
import { ToolbarButtonGroup, ToolbarButton } from '../../primitives/Toolbar'
...
import { ZoomInIcon, ZoomOutIcon } from '@radix-ui/icons'
...
const timeseriesZoomAtom = atom(1)

export const Zoom = () => {
    const [zoom, setZoom] = useAtom(timeseriesZoomAtom)

    const zoomOptions = [
        { id: 0, value: 'magnify', textValue: 'zoom in', alt: undefined, icon: <ZoomInIcon /> },
        { id: 0, value: 'reduce', textValue: 'zoom out', alt: undefined, icon: <ZoomOutIcon /> },
    ];

    const handleChang = (event: React.ChangeEvent<HTMLButtonElement>) => {
        setZoom(event.currentTarget.value)
    }

    return (
        <ToggleGroup value={zoom} onValueChange={handleChange}
            {zoomOptions.map((zoomOption, index) => (
                <Tooltip content={zoomOption.textValue.toUpperCase()}>
                    <ToolbarButton
                        key={index}
                        value={zoomOption.Value}
                        onClick={handleClick}
                    >
                        {zoomOption.icon}
                    </ToolbarButton>
