...
export const GraphDetailsAnnotation = ({ details }: { details: IGraphDetailsProps }) => {
    ...
    const { start, end, durationInMs, numIntervals, tickSizeInMs, cummulativeClicks, nonEmptyDatums, allDatums }: IGraphDetailsProps = details;

    const durationStr = `${durationInMs/(1000*60*60*24)} days`
    const intervalLenStr = `${tickSizeInMs/(1000*60*60*24)} days`
    const globalMean = `Mean: ${parseFloat(cummulativeClicks/emptyDatums)}`
    const cherryPickedMean = `Cherry Picked Mean: ${parseFloat(cummulativeClicks/nonEmptyDatums)}`
