...
const SecondaryChart: React.FC<SecondaryChartProps> = ({
    ...
    let isReady = !loading && !error && clicks?.length 

    const data: Datum[] = isReady && clicks.map((click: { 
        x: number; 
        y: number 
    }, index: number) => {
        
        let timestamp = formatClickDate(click.x, start, interval).timestamp
        return {
            index,
            timestamp: import { filteredDataAtom, boundsAtom } from '../../atoms/timeseries'
            clickscore: click.y,
    ...
    let { start, end }:{ start: number; end: number; } = { 
        start: minTimestamp || new Date(new Date().getFullYear(), 1, 1), 
