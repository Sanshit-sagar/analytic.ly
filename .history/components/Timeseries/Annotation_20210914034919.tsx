...
import { useGloballyConsistentColors } from '../../hooks/'
...
export const GraphDetailsAnnotation = ({ details }: { details: IGraphDetailsProps }) => {
    ...
    const rangestr = `${formattedDate(getDate(detailedStats[0].value))} to ${formattedDate(getDate(detailedStats[1].value))}`
    const minimaxstr = `${durationStr} | max: ${maxScore} | min: ${minScore}`
    const intervalsAndMean = `${numIntervals} | avg: ${globalMean}` 
    ...
    return (
        ...
        >
            ...
            <Label
                showBackground={true}
                title={'Statistics'}
                subtitle={`${rangestr} ${minimaxstr} | intervalsAndMeanSt`}
