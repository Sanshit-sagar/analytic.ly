...
function formatClickDate(x: number, mints: number, interval: string): string | undefined {
    let x0: number = mints;
...
const Chart = ({ 
  ...
  const dataA: Datum[] = new Array(clicks.length).fill(null).map((_, i) => ({
    ...
    timestamp: format(formatClickDate(clicks[i].x, minTimestamp, interval), 'dayhourmin'),
  }));
