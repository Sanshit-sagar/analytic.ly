...
const SecondaryChart: React.FC<SecondaryChartProps> = ({
    ...
    let { start, end, durationInMs, tickSizeInMs, numIntervals }: GraphDetails = details;
    ...
    const { filteredDataState: { setFilteredData, setBounds } } = useContext(MarketContext);
    ...
    useEffect(() => {
        if (data.length) {
            ...
            setBounds({ x0: getDate(data[0]).getTime(), y0: -2, x1: getDate(data[data.length-1]).getTime(), y1: 2})
        }
