...
export interface RankedDatum {
    { title: string; score:number; rank: number; normalizedFreq: number; }
}
...
export const useUserRankings = (category: string, email?: string, topOnly: boolean = false) => {
    ...
    if(data && !error && topOnly) {
        if(category==='frequencies') {
            return data?.sort((a: Datum ,b: RankedDatum) => b.score - a.score)[0] || null
        } else {
            return data?.uniques[0] || null 
        }
    }
...
export const useUserStatistics = () => {
    let endpoint = `http://localhost:3000/api/users/sanshit.sagar@gmail.com/statistics`
    const { data, error } = useSWR(endpoint, fetcher)
