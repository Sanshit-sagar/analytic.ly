...
export const useGloballyConsistentColors = () => {
    ...
    let darkMode = useAtomValue(darkModeAtom)
    let activeThemeId = useAtomValue(activeThemeIdAtom)
    let activeThemeClassName = useAtomValue(activeThemeObjectAtom)

    let activeThemeColors = activeThemeClassName
    return {
        THEME: JSON.stringify(activeThemeClassName)
    }
