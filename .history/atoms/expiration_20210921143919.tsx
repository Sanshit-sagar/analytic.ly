...
export const expirationResultsAtom = atom(
    (get) => {
        ...
        }=== StageTypeEnum.UNINITIALIZED ? 'n/a' : get(stageAtom)===StageTypeEnum.START ? get(hoveredStartDateAtom)?.toLocaleDateString() : `${get(hoveredStartDateAtom)?.toLocaleDateString()} to ${get(hoveredEndDateAtom)?.toLocaleDateString()}`;
    }
)
