...
export const recalcDataAtom = atom(
    ...
    (get, set, update: Datum[]) => {
        ...
        set(boundsAtom, {
            x0: new Date(update[0].clickdate).getTime(),
        ...
        let runningTotal = 0; 
        let windowedData: Datum[] = [];
        let windowSize =  get(windowSizeAtom)

        set(movingAvgDataAtom, update.map((datum: Datum, index: number) => {
            runningTotal += datum.clickscore
            if(index >= windowSize) {
                runningTotal -= windowedData[index - windowSize].clickscore
            }
            return {
                ...datum,
...
export const activeFilteredDataAtom = atom(
    (get) => {
        let selectedStat get(statisticAtom).toLowerCase().startsWith('timeseries') ? get(filteredDataAtom) 
        : get(statiget(cummulativeDataAtom) 
);
