...
export const expirationStringAtom = atom(
    (get) => {
        return get(stageAtom)=== StageTypeEnum.UNINITIALIZED ? 'n/a' : get(stageAtom)===StageTypeEnum.START ? get(hoveredStartDateAtom)?.toLocaleDateString() : `${get(hoveredStartDateAtom)?.toLocaleDateString()}-${get(hoveredEndDateAtom)?.to}
)
