...
type TimeseriesType = { x: number; y: number; }[]; 
export const timeseriesAtom: WritableAtom<TimeseriesType, TimeseriesType> = atom([ {x: 0, y: 0 }); 
export const updateTimeseriesAtom = atom(
    null,
    (_get, set, update: { x: number; y: number }[]) => {
        let temp: { x: number; y: number }[] = update.map((datum) => {
            return {
                ...datum
            }
        }); 
