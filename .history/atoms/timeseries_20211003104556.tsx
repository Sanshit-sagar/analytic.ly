...
type TsDatum = { x: number; y: number; }
...
export const updateTimeseriesAtom: = atom(
    null,
    (_get, set, update: { data: RawData, minTimestamp: number, interval: string }) => {
        
        let temp: Timeseries = update.data.map((datum: TsDatum, index: number) => {
            return { 
                index, 
                timestamp: formatClickDate(datum.x, update.minTimestamp, update.interval).timestamp,
                clickscore: datum.y, 
                clickdate: new Date(formatClickDate(datum.x, update.minTimestamp, update.interval).timestamp) 
            }
        });
