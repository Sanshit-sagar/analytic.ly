...
function toTimestampStr(date: Date | undefined) {
    return isValidDate(startDate) ? `${new Date(startDate).getTime()}` : ''
}
...
export const expirationTimestmapsAtom = atom(
    (get) => {
        switch(get(stageAtom)) {
            ...
            case(StageTypeEnum.START): {
                let startDate = get(hoveredStartDateAtom)!==undefined ? new Date(get(hoveredStartDateAtom)).getTime() : ''
                return isValidDate(startDate) ? `${new Date(startDate).getTime()}` : ''
            }
            case(StageTypeEnum.END): {
                return `${format(get(hoveredStartDateAtom))} to ${format(get(hoveredEndDateAtom))}`
