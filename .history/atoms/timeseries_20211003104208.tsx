...
export const updateTimeseriesAtom: WritableAtom<null, = atom(
    null,
    (_get, set, update: { data: RawData, minTimestamp: number, interval: number }) => {
        if(!data?.length || !minTimestamp || !interval) return null

        let temp: Timeseries = update.data.map((datum: Datum, index: number) => {
            return { 
                index, 
                timestamp: formatClickDate(datum.x, minTimestamp, interval).timestamp,
                clickscore: datum.y, 
                clickdate: new Date(formatClickDate(datum.x, minTimestamp, interval).timestamp) 
            }
        })
        set(timeseriesAtom, [...temp])
    }
);
