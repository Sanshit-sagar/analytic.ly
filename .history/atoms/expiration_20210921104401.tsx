import { WritableAtom, Atom, atom, WritableAtom } from 'jotai'
...
// only start -> '__' 
// start + end -> '___' - '__'
type StageType = 'uninit' | 'hovering_start' | 'start' | 'end' | 'hovering_end' | 'reset'
enum StageTypeEnum {
...
const stageAtom = WritableAtom<StageType, React.SetStateAction<Date>>(
    (get) => {
        if(get(endSelectionCompletedAtom)!==undefined) return StageTypeEnum.
        else if(get(startSelectionCompletedAtom)!==undefined) return 'started'
        else return 'uninit'
    },
    (get: StageType, set: React.SetStateAction<Date>, updatedDate: Date) => {
