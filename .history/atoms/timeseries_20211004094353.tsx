...
export const windowSizeAtom = atom((get) => get(filteredDataAtom).length / 25) 
...
export const recalcDataAtom = atom(
    ...
    (get, set, update: Datum[]) => {
        ...
        set(boundsAtom, {
            x0: update?.length ? new Date(update[0].clickdate).getTime() : new Date(2021,1,1).getTime(),
            y0: update?.length ? update[0].clickscore : 5,
            x1: update?.length && update ? new Date(update[1]?.clickdate).getTime() : new Date().getTime(),
            y1: update?.length ? update[1].clickscore : 10,
        }); 

        // let runningTotal = 0; 
        // let cummClicks: Datum[] = [...Array(update.length)];
        // let windowSize =  update.length/5
        // let ptr = 0
        // let i = 0
        // for(ptr = 0; ptr < update.length; ptr++) {
        //     runningTotal += update[ptr].clickscore
            
        //     if(ptr > windowSize) {
        //         runningTotal -= cummClicks[ptr - windowSize].clickscore
        //     };

        //     cummClicks.push({
        //         index: i++,
        //         clickscore: runningTotal,
        //         timestamp: update[ptr].timestamp,
        //         clickdate: update[ptr].clickdate,
        //     });
        // }
        // set(movingAvgDataAtom, cummClicks)
    }
);
