...
export const windowSizeAtom = atom((get) => {
        return 0.3 * (get(boundsAtom).x1 - get(boundsAtom).x0)
    }
)

let 3monthsTime = 1000 * 60 * 60 * 24 * 7 * 30 * 3
export const scaleFactorAtom = atom(0.5)
export const scaleFactor = aotom 
export const windowerAtom = atom(
    (get) => {
        return 3monthsTime / get(scaleFactor);
    }
);

export const resizeWindowAtom = atom(
    null,
    (get, set, update: number) => { // update = window size is now 20% of domain
        let runningTotal = 0
        let clickscore = 0//
        let start = 0

        set(windowSizeAtom, update)
        set(movingAvgDataAtom, get(filteredDataAtom).map((filteredData, i) => {
            runningTotal += filteredData[i].clickscore

            if(i > get(windowSizeAtom) {
                runningTotal -= filteredData[i-get(windowSizeAtom)].clickscore
                start++;
            }
