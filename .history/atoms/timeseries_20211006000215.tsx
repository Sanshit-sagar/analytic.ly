...
export const windowSizeAtom = atom(
    (get) => get(filteredDataAtom).length / 3),
    (_get, set: React.SetStateAction<Datum[]>, update: Datum[]) => {
    set(windowSizeAtom, update.map((updatedValue) => ({ 
        ...updatedValue 
    })));
    }
)
...
export const resizeWindowAtom = atom(
    ...
    (get, set, update: number) => { // update = window size is now 20% of domain
        ...
        set(movingAvgDataAtom, get(filteredDataAtom).map((filteredData, i: number) => {
