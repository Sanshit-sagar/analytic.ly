...
export const expirationResultsAtom = atom(
    (get) => {
         get(stageAtom)=== StageTypeEnum.UNINITIALIZED ? 'n/a' : get(stageAtom)===StageTypeEnum.START ? get(hoveredStartDateAtom)?.toLocaleDateString() : `${get(hoveredStartDateAtom)?.toLocaleDateString()} to ${get(hoveredEndDateAtom)?.toLocaleDateString()}`;
    }
)
