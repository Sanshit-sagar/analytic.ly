...
export const resizeWindowAtom = atom(
    ...
    (get, set, update: number) => { // update = window size is now 20% of domain
        if(update < 0 || update > 30) return 
        let runningTotal = 0
        let clickscore = 0//
        let start = 0

        set(windowSizeAtom, update)
        set(movingAvgDataAtom, get(filteredDataAtom).map((datum, i: number) => {
            runningTotal += datum[i].clickscore

            if(i > get(windowSizeAtom) {
                runningTotal -= datum[i-get(windowSizeAtom)].clickscore
                start++;
            }
