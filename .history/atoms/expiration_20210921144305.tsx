    ...
    (get) => {
        switch(get(stageAtom)) {
            ...
            case(StageTypeEnum.END): {
                return `${format(get(hoveredStartDateAtom))}${format(get(hoveredEndDateAtom))
            }
        }
        ? 'n/a' : get(stageAtom)===StageTypeEnum.START ? get(hoveredStartDateAtom)?.toLocaleDateString() : `${get(hoveredStartDateAtom)?.toLocaleDateString()} to ${get(hoveredEndDateAtom)?.toLocaleDateString()}`;
    }
)
