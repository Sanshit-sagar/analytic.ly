...
const pwValidAtom = atom((get) => `${get(isPasswordValidAtom) ? 'ye' : 'ne'}`)
export const passwordResultsAtom = atom(
    (get) => `Password (${get(pwValidAtom)}): ${get(passwordAtom).substring(0,8)}${get(passwordAtom).length > 8 ? '...' : ''}`
)
