...
const SectionLabelFieldset = styled('fieldset', {
    height: '100%', 
    width: '80%', 
    display: 'flex', 
    fd: 'column', 
...
const SectionLabelName = styled('label', {
    fontWeight: 'bolder', 
    ...
    return (
        <SectionLabelFieldset>
            <SectionLabelName {...props}>
                <> {href?.length  ? 
                    <a href={href}>  {sectionName}S </a> : {sectionName}} </>
                <> {icon || ''} </>
            </SectionLabelName>
...
export const ListBoxItemDescription = ({ slot, props }: IListBoxItemProps) => (
    <StyledLabel {...props}><StyledDescription> {slot} </StyledDescription> </Label>
);
