...
const CustomHeading = ({ children, ...props }: CustomHeadingProps) => {
    return (
        <Text size='1' css={{
            width: '100%', color: '$text', fontWeight: 350, display: 'flex', fd: 'row', jc: 'space-between', ai: 'center', gap: '$2', '&:hover': { fontWeight: 500 } 
        }}>
            {children}
        </Text>
    );
...
const StyledHeading = styled(CustomHeading, {
    margin: '$1',
    width: '100%', color: '$text', fontWeight: 350, display: 'flex', fd: 'row', jc: 'space-between', ai: 'center', gap:
    '&:hover': {
        color: '$accentHover',
    },
})
