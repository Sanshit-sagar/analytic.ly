...
const ClicksTable = ({ isLoadingInitialData, isLoadingMore, isEmpty, isReachingEnd, isRefreshing, clicks, cursor }: ITabulatedClicks) => {
    if(isLoadingInitialData || isLoadingMore) return <Text> Loading... </Text>
    
    if(isReachingEnd || isRefreshing) {
        return <Text> Status: {isReachingEnd ? 'nearing end' : isRefreshing ? 'refreshing' : ''} </Text>
    }
...
PaginatedClicks.getLayout = function getLayout(page: any) {
