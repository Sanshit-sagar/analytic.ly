...
export default getHandler()
    ...
    .get('/api/clicks/recents/:time/:unit',  async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(time && unit && (unit==='hours' || unit==='mins' || unit==='secs') && multiplier!==-1) {
            let timeago: number = parseInt(`${time}`) || 0
            timeago *= multiplier*timeago

            let startTimestamp: number =new Date(new Date().getTime() - timeago).getTime() - 100; 
        
            // res.status(200).json({ message: `Getting clicks from the past ${timeago} * ${multiplier} secs`})
            // const timespanStart = new Date().getTime() - timeago*multiplier;
            const clickstream = await getClickstreamStartingAt(startTimestamp, isAsc);
