...
export default getHandler()
    .get('/api/clicks/slug/:slug', async (req: NextApiRequestExtended, res: NextApiResponse) => {
    ...
    .get('/api/clicks/paginate/:email/:cursor/:amount', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        const cursor: number = parseInt(req.params.cursor || '0')
        const amount: number = parseInt(req.params.amount || '10')

        if(email) {
            const { query, start, clickstream }: IPagedResults = await paginateClickstream(email, cursor, amount)

            const cursors: ICursors = { 
                id: clickstream[clickstream.length - 1].index, 
                timestamp: parseInt(clickstream[clickstream.length - 1].timestampRaw) 
            }
            res.status(200).json({ query, data: clickstream, cursors, start })
        } else {
            res.status(403).json({ BAD_REQUEST_INVALID_EMAIL })
        }
    });