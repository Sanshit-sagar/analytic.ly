...
export default getHandler()
    ...
    .get('/api/clicks/filter/:name/value/:value', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(filterName==='cfRay' && doesExist(filterName) && doesExist(filterValue)) {
            ...
            const clickstream = await getClickFromField(filterName, filterValue);
           
            if(clickstream && Object.entries(clickstream)?.length) {
                res.status(200).json({ clickstream }); 
            } else {
                res.status(200).json({ error: 'NO MATCH FOUND', clickDetails: {} })
            }
        } else {
    ...
    .get('/api/clicks/range/:start/to/:end', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(!start && !end) {
            ...
            if(clickstream?.length) {
                ...
                res.status(200).json({ clicks: [], error: 'No Matches found for the given range' }); 
            }
        }
    })
    ...
    .get('/api/clicks/recents/:time/:unit',  async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(time && unit && (unit==='hours' || unit==='mins' || unit==='secs') && multiplier!==-1) {
            ...
            const clickstream = await getDoubleEndedClickstream(startTimestamp, currentTimestamp, isAsc);

            if(clickstream?.length) {
                res.status(200).json({ clickstream, startTimestamp })
            } else {
                res.status(200).json({ clickstream: [], error: `No matches found for the given range` })
            }
        } else {
