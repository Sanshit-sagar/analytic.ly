...
export default getHandler()
    ...
    .get('/api/clicks/paginate/:email/:cursor/:amount', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(email) {
            const { query, start, end, clickstream }: IPagedResults = await paginateClickstream(email, cursor, amount)
            
            console.log(`QUERY: ${query}--CURSOR: ${cursor} -- AMOUNT: ${amount}`)
            
            const cursors: ICursors = { id: end, timestamp: parseInt(clickstream[end].timestampRaw) }
            res.status(200).json({ query, page: clickstream, cursors, start })
        } else {
