...
export function getDateFromArray(dateArr: string[]): Date | string | null {
    let mm = parseInt(dateArr[0]);
    let dd = parseInt(dateArr[1]);
    let yy = parseInt(`20${dateArr[2]}`);

    return `${mm}-${dd}-${yy}`;

    // if(mm >= 0 && mm <= 12 && dd >= 1 && dd <= daysInMonth(mm) && yy>=20 && yy<=25) {
    //     let dateObj = new Date()
    //     dateObj.setUTCFullYear(yy, mm, dd); 
    //     return dateObj;
    // } else {
    //     return null;
    // }
...
export default getHandler()
    ...
    .get('/api/clicks/by/date/:date', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        const date: Date | string | null = getDateFromArray(dateArr);

        if(dateArr.length !== 3 || !date) {
            res.status(403).json({ error: 'Bad Request, Invalid date format. please use: mm-dd-yy'}); 
        } else {
            res.status(200).json({ message: `You specified the date: ${date.toLocaleString()}` });
        }   
        // getClickstreamOnDate
    });