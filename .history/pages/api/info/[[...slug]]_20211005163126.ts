...
function getClickstream(slug: string): Promise<ClickstreamDetails> {
    return new Promise((resolve, reject) => {

        const clickstream = await getDoubleEndedClickstream(0, -1, slug, 'slug', true)

        if(clickstream?.views?.length) {
            resolve(clickstream)
        } else {
            reject(new Error(`No data for the slug: ${slug}`))
        }
    })
}

function formatClickstream(slug: string, amount: number, interval: string, range: string): Promise<ClickstreamDetails> {
    Promise.resolve(const clickstream: BoundedClickstream = await getClickstream(slug).then((response) => {
        const boundaries: number[] = getRangeBoundaries(clickstream.amount, clickstream.range, clickstream.interval)
        ...
        reject(new Error(`Error in formatting clickstream for slug: ${slug}`))
    });
}
...
export default getHandler()
    ...
    .get('/api/info/clickstream/:slug/:amount/:range/:interval', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(slug?.length) {
            const data = getClickstream(slug)
