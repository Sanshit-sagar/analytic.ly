...
export default getHandler()
    .get('/api/info/:slug1/:slug2', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            ...
            if(!slug1?.length || !slug2?.length) {
                ...
                slugs.map((slug: string, index: number) => {
                    promises.push(isSlugOwnedByUser(slug, email));
                    promises.push(isSlugVisited(slug)); 
                })
