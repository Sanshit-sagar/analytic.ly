...
function getClickstream(slug: string): Promise<BoundedClickstream> {
    return new Promise((resolve, reject) => {
        ...
        if(clickstream && clickstream.minTimestamp && clickstream.maxTimestamp) {
            resolve(clickstream)
        } else {
...
function formatClickstream(slug: string, amount: number, interval: string, range: string): Promise<ClickstreamDetails> {
    let p: Promise<BoundedClickstream> = Promise.resolve(getClickstream(slug))

    return p.then((clickstream: BoundedClickstream) => {
        ...
        console.error(error)
        throw new Error(`Couldn't format clickstream for slug: ${slug}. Error: ${error?.message || ''}`)
    })
}
