...
function getClickstream(slug: string, amount: number, interval: string, range: string): ClickstreamDetails {
    
    return new Promise((resolve, reject) => {
        const boundedClicks: BoundedClickstream = await getDoubleEndedClickstream(0, -1, slug, 'slug', true)
        const { views, minTimestamp, maxTimestamp }: BoundedClickstream = await getDoubleEndedClickstream(0, -1, slug, 'slug', true)
    })
}

async function formatClickstream(slug: string, amount: number, interval: string, range: string): Promise<ClickstreamDetails> {
   
    const { views, minTimestamp, maxTimestamp }: BoundedClickstream =
