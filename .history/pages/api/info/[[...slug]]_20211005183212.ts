...
export default getHandler()
    ...
    .get('/api/info/differential/:amount/:range/:interval/slugs/:slug1/:slug2', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        let email = 'sanshit.sagar@gmail.com'
        ...
        if(slug1 && slug2) {
            Promise.all(promises).then((result) => {
                ...
                console.error(error);
                res.status(500).json({ error: `Unkown error ${error?.message || ''}`})
            });
        } else {
