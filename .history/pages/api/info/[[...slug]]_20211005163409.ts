...
function getClickstream(slug: string): Promise<BoundedClickstream> {
...
function formatClickstream(slug: string, amount: number, interval: string, range: string): Promise<ClickstreamDetails> {
    Promise.resolve(await getClickstream(slug).then((clickstream) => {
        ...
        const { mergedIntervals, viewsByIntervals, bounds, numPeriods, numClicks } = merge(clickstream.views, boundaries, ticksInRange, interval)
        ...
        throw new Error(`Error in formatting clickstream for slug: ${slug}`)
    })
}
