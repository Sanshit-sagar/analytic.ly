...
export default getHandler()
    ...
    .get('/api/info/clickstream/:amount/:range/:interval/slug/:slug1/:slug2', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        let promises = []
        let responses: Map<string, ClickstreamDetails> = new Map<string, ClickstreamDetails>();

        slugs.map((slug: string, index: number) => {

            promises.push(new Promise((resolve, reject) => {
                await getFormattedClickstream({
                     slug, amount, range, interval 
                }).then((response) => {
                    return resolve(responses.set(slug[index], response))
                }).catch((error) => {
                    ...
                    for(j = index; j <= slugs.length; j++) {
                        responses.set(slugs[j], {
                            clickstream: [],
                            partitions: [],
                            timestampsDomain: { start: new Date(1970,1,1).getTime(), end: new Date().getTime() },
                            partitionsDomain: { start: new Date(1970,1,1).getTime(), end: new Date().getTime() },
                            clickCount: 0,
                            partitionCount: 0,
                        })
                    }
