...
function getFormattedClickstream(props: RawClickstream): ClickstreamDetails => {

    const boundaries: number[] = getRangeBoundaries(props.amount, props.range, props.interval)
    const ticksInRange: number[] = getTicksInRange(boundaries[0], boundaries[1], props.interval, props.interval)
    const { mergedIntervals, viewsByIntervals, bounds, numPeriods, numClicks } = merge(props.views, props.boundaries, ticksInRange, props.interval)
