...
const extract = (payloads: ResponsePayload[], property: keyof ResponsePayload): UnionList<number,string> => {
    return payloads.map((payload: ResponsePayload, i: number) => (typeof property === 'number'
                ?    payload.hasOwnProperty(property)    ?   parseInt(`${payloads[i][property]}`)   :    typeof property === 'string'                   
                ?    payload.hasOwnProperty(property)     
                :    new Date(parseInt(`${payloads[i][property]}`)).getTime()   ? 'breadpakoda' 
