...
function tldr(untypedData: (ExtractedNumberProperty | ExtractedStringProperty)[]): SummaryStats<(ExtractedStringProperty | number)> {
    let data: number[] = untypedData.map(ud => +ud);
    let extent: [undefined, undefined] | [number, number] = d3.extent(data)
