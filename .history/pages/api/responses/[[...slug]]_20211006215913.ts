...
function tldr<T>(untypedData: T[]): SummaryStats<T> {
    if(typeof untypedData !== 'number' && typeof untypedData !== 'string')
        return { min: undefined, max: undefined, mean: undefined, variance: undefined, median: undefined, deviation: undefined }
    }

    let data: T[] = untypedData.map((ud: T) => +ud)
