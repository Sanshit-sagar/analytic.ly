...
const extract = (payloads: ResponsePayload[], property: keyof ResponsePayload) => {
        payloads.map((payload: ResponsePayload, i: number) => 
                typeof property === 'number'      ?   payload.hasOwnProperty(property) ? parseInt(`${payloads[i][property]}`)               
            :   typeof property === 'string'      ?   payload.hasOwnProperty(property) 
                    :  new Date(parseInt(`${payloads[i][property]}`)).getTime() 
            ?    payload.  :   'breadpakoda'                               :  payload.hasOwnProperty(property)                                              
