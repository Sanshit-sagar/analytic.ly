...
import { min, max, mean, median, variance, deviation } from 'd3-array'
...
const extract = (payloads: ResponsePayload[], property: keyof ResponsePayload): (number | string)[] => {
    return payloads.map((payload: ResponsePayload, i: number) => {
        return typeof property === 'number'  ?  payload.hasOwnProperty(property) ? parseInt(`${payloads[i][property]}`) : -1 
        :       typeof property === 'number' payload.hasOwnProperty(property) String(payloads[i][property])
                : 
                ?   -1 : ''
...
function tldr(untypedData: (number | string)[]): SummaryStats<number> {
...
export default getHandler()
    ...
    .get('/api/responses/user/:email/times', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            ...
            const dates = extract(responses, 'date') 
            const stats = tldr(data)
            res.status(200).json({ data, dates, stats })
