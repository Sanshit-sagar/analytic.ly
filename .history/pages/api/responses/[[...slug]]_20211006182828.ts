...
type ResponseHeaderPropertyType = 'resp'
const extract = (payloads: ResponsePayload[], property: keyof ResponsePayload): (number | string)[] => {
    return payloads.map((payload: ResponsePayload, i: number) => {
        return typeof property === 'number'  
                ?   payload.hasOwnProperty(property) ? parseInt(`${payloads[i][property]}`) : -1 
...
function extractZipAndSummarize(payload: ResponsePayload[]) {
    const data = extract(payload, 'responseTime') 
    const dates = extract(payload, 'date') 
