...
function extract(payloads: ResponsePayload[], property: keyof ResponsePayload): (number | undefined)[] {
    return payloads.map((payload: ResponsePayload, i: number) => {
        return payload.hasOwnProperty(property) ? parseInt(`${payloads[i][property]}`) : undefined; 
    })
}
