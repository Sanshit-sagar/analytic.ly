...
async function getVerboseUserSlugConfigs(email: string) {
    let slugDetailsRaw = await hgetallQuery(`slug.details.by.user.${email}`)
    let slugDetails = JSON.parse(userSlugsRaw)

    let sanitizedDetailedSlugs =  slugDetails ? slugDetails?.map((datum: string, i: number) => {
        if(i%2===1 && i+1 < slugDetails.length) {
            let fields = slugDetails[i+1]
            return {
                key: `slug-idx-${(i-1)/2}`, 
                slug: datum,
                ...fields
