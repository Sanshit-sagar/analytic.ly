...
async function hgetallQuery(key: string) {
    return await redis.hgetallQuery(`${key}`,)
}
...
async function getSlugConfigurationsByUser(email: string) {
    return await lRangeQuery(`slugs.by.user.${email}`)
}
async function getDetailedUserSlugConfigs(email: string) {
    return await hgetallQuery(`slug.details.by.user.${email}`)
}
...
export default getHandler()
    ...
    .get('/api/config/slug/:slug', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        let user = await users.getUser(req.session.userId)
        let { email } = getUserDetails(user)
