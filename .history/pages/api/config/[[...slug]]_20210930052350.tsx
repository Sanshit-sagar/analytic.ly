...
async function getVerboseUserDestinationsPromises(email: string) {
    ...
    return { destinations, promises, mssg: 'hi' }
}
...
export default getHandler()
    ...
    .get('/api/config/destinations/:verbosity', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        let destSlugMap: { [key: string]: string[] } = {}
        Promise.all(promises)
            .then((response: any) => {
                destinations.map((dest: string, i: number) => destSlugMap[dest] = [...response[i]]
                res.status(200).json({ destSlugMap })
            })
