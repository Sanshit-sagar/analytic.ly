...
export default getHandler()
    ...
    .get('/api/config/slugs/:verbosity', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            let descriptorLevel = 'concise'
            let verbosity = req.params.verbosity 
            if(verbosity!=='verbose') {
                descriptorLevel = 'concise'
                data = getConciseUserSlugConfigs(userDetails.email)
            } else {
                descriptorLevel = 'verbose'
                data = getVerboseUserSlugConfigs(userDetails.email)
            }
            res.status(200).json({ data, resource: 'Slugs', user: userDetails.email, descriptor: 'verbosity', level: descriptorLevel })
        } catch(error) {
