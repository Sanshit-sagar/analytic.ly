...
async function getSlugConfigurationsByUser(email: string) {
    let configsAndSlugs = await lRangeQuery(`slugs.by.user.${email}`)

    console.log(configsAndSlugs)
   
    return configsAndSlugs
}
...
export default getHandler()
    .get('/api/config/s', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
    ...
    .get('/api/destinations', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        let user = await users.getUser(req.session.userId)
        let { email } = getUserDetails(user)

        let destinations = await getDestinationsByUser(email)
