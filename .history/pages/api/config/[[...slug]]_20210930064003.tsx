...
import { useDateFormatter } from '@react-aria/DateFormatter'
...
function decodeExpiration(exp: string) {
    let expArr = exp.split('&')
    const formatter = useDateFormatter()
    
    return [
        formatter.format(new Date(parseInt(expArr[0]?.split('=')[0] ?? '')),
        formatter.format(new Date(parseInt(expArr[1]?.split('=')[1] ?? '')),
    ];
}
