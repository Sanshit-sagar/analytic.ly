...
async function postSlugConfiguration(data: IPostProps) {
    ...
    pipeline.exec((_, results: any) => {
        if(results) {
            console.log(results)
        } else {
            console.log('naathing to show')
        }
    });
...
export default getHandler()
    .get('/api/config/list/:cursor/:pageSize', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            const cursor = Math.max(parseInt(req.params.cursor), 0) // earliest the cursor can be is 0
            const pageSize = Math.max(parseInt(req.params.pageSize), 5) // all queries must be for >= 5 items 
        
            const userSlugConfigs = await getSlugConfigurationsByUser(email, 0, pageSize) 

            res.status(200).json({ 
                data: userSlugConfigs.slice(cursor, cursor + pageSize), 
                previous: cursor, 
                cursor: next,
