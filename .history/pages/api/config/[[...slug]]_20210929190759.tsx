...
export default getHandler()
    .get('/api/config/list/:cursor/:pageSize', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            ...
            res.status(200).json({ 
                data: userSlugConfigs.slice(0, pageSize), 
                previous: cursor, 
                cursor: Math.min(cursor + pageSize, userSlugConfigs[userSlugConfigs.length - 1],
                next: userSlugConfigs[userSlugConfigs.length]
            })
        } catch(error) {
