...
async function getVerboseUserDestinationsPromises(email: string) {
    let slugsForDestinationMap: { [key: string]: string[]; } = {} 
    let promises: Promise<any>[] = [];

    let destinations = await getAllUserDestinations(email)
    
    destinations.map(async (dest: string, i: number) => {
        promises.push(new Promise(function(resolve: any, reject: any) {  
            redis.zrange(`slugs.for.destination.${dest}.by.user.${email}`, 0, -1)
            .then((response: any) => {
                slugsForDestinationMap[dest] = []
                return resolve(slugsForDestinationMap[dest] = [...response]); 
            })
            .catch((error: any) => {
                console.error(error)
                return reject(error)
...
async function getVerboseUserDestinations(emai: string) {
    let promises: Promise<any> = await getVerboseUserDestinationsPromises(email)
...
export default getHandler()
    ...
    .get('/api/config/destinations/:verbosity', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        let descriptorLevel: VerbosityType = 'verbose'
        console.log('ABOUT TO CALL')
        let data = await getVerboseUserDestinations(email)
        console.log('BACK FROM THE CALL WITH')
        console.log(data)
