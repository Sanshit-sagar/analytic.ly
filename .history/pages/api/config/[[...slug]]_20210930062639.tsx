...
async function getVerboseUserSlugConfigs(email: string) {
    ...
    // let verboseUserSlugs = userSlugs ? Object.keys(userSlugs).map((datum: string, i: number) => {
    //     if(i%2===1 && i+1 < userSlugs.length) {
    //         let fields = JSON.parse(userSlugs[i+1])
    //         return {
    //             key: `slug-idx-${(i-1)/2}`, 
    //             slug: datum,
    //             ...fields
    ...
    //     } 
    // }) : []; 
    return userSlugs ? Object.entries(userSlugs?.data) : null
}
