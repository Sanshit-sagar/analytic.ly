...
async function getVerboseUserDestinationsPromises(email: string) {
    ...
    return { destinationsByIndex: Object.keys(slugsForDestinationMap), promises }
}
...
export default getHandler()
    ...
    .get('/api/config/destinations/:verbosity', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        let {destinationsByIndex,promises} = await getVerboseUserDestinationsPromises(email)

        Promise.all(promises)
            .then((response: any) => res.status(200).json({ response, destinationsByIndex }))
