...
async function postSlugConfiguration(data: IPostProps) {
    ...
    const prpomise = redis.pipeline()
        .set(`slug.${data.slug}.details`, slugDetails)
        .lpush(`slugs.by.user.${data.email}`, slugDetails)
        .lpush(`destinations.by.user.${data.email}`, slugDetails) 
        .set(`destination.for.slug.${data.slug}`, data.destination)
        .zadd(`slugs.by.user.${data.email}.autocomplete`, 0, data.slug)
        .zadd(`destination.by.user.${data.email}.autocomplete`, 0, data.destination) 
        .zadd(`slugs.for.destination.${data.destination}.by.user.${data.email}`, 1, data.slug) 
