...
async function getVerboseUserSlugConfigs(email: string) {
    let userSlugsRaw = await hgetallQuery(`slug.details.by.user.${email}`)
    let userSlugs = userSlugsRaw ? JSON.parse(userSlugsRaw.data)

    let verboseUserSlugs =  userSlugs ? userSlugs?.map((datum: string, i: number) => {
        if(i%2===1 && i+1 < userSlugs.length) {
            let fields = userSlugs[i+1]
            return {
                key: `slug-idx-${(i-1)/2}`, 
                slug: datum,
                ...fields
