...
import { requireSession, getUser } from '@clerk/nextjs/api'
...
export default getHandler()
    ...
    .get('/api/users/ranked/views', requireSession(async (req: NextApiRequestExtended, res: NextApiResponse) => {
    ...
    .get('/api/users/:user/frequencies', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(email) {
            let categories: string[] = ['browser', 'os', 'engine', 'httpProtocol', 'tlsVersion', 'country', 'ip'];
            let promises: Promise[] = [];
            let uniqueMap: any = {};
            ...
            categories.map((category: string, _: number) => {
                promises.push(new Promise(function(resolve: any, reject: any) {
                    ...
                    .then((response: any) => {
                        ...
                        // TODO: resolve [] if response is null or error and log to console.error
                        let rankings: ScoredSlug[] | string[] = formatAndSortSlugs(frequencies); 
    ...
    .get('/api/users/:user/summary', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        const email: string = req.params.user
        if(email) {
            try {
                const userSummary = await getSummaryForUser(email);
    ...
    .get('/api/users/:user/statistics', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        const email: string = req.params.user

        if(email) {
            try {
                const userStatistics = await getStatisticsForUser(email);
