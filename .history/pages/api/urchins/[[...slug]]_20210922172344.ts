...
import { 
    ...
    getAllUrchinsForUserByCategory
...
export default getHandler()
    ...
    .get('/api/urchins/user/:email/utmKey', async (req: NextApiRequestExtended, res: NextApiResponse) => {
            const email = req.params.email
            const category = 

            if(email) {
                try {
                    const utmParameters = await getAllUrchinsForUserByCategory(email, 
                    res.status(200).json({ utmParameters })
                } catch (error) {
                    res.status(500).json({ error })
                }
            }
