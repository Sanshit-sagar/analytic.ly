...
async function suggestRandomUniqueSlug(saltLength: number, numStrings: number, saltType: string, separator: string, isManly: boolean) {
...
type NamorDataType = string" | "number" | "mixed" | undefined'

function suggestRandomSlug(saltLength: number = 5, numStrings: number = 2, saltType: string | undefined, separator: string | undefined, isManly: boolean = false) {
    numStrings =  numStrings>5 ? Math.max(numStrings, 8) : numStrings<2 ? 2 : numStrings
    return isManly
                ? namor.generate({ words: numStrings, saltLength, saltType, separator, subset: `${MANLY}` })
                : namor.generate({ words: numStrings, saltLength, saltType, separator, subset: `${MANLY}` })
}
...
function charify(separatorInput: string | string[] | null | undefined) {
    return separatorInput?.length 
            ...
            ? separatorInput.charAt(0) : separatorInput.join('').charAt(0)
            : '-'
}
...
function saltypify(saltTypeInput: string | string[] | null | undefined) {
    return saltTypeInput?.length 
            ...
            ? saltyBelongsTo(saltTypeInput) : saltyBelongsTo(saltTypeInput.join('')) 
            : 'mixed'
}
