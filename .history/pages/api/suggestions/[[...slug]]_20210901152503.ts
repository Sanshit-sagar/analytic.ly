...
function suggestRandomUniqueSlug(saltLength: number, numStrings: number, manly: boolean) {
    let suggestedRandomSlug = suggestRandomSlug(saltLength, numStrings, manly);
    if(redis.hget(`slug.${suggestedRandomSlug}`))
}
