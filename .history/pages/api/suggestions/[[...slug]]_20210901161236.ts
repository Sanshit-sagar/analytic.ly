...
function suggestRandomSlug(saltLength: number = 5, numStrings: number = 2, saltType: SaltType, separator: string | undefined, isManly: boolean = false) {
    ...
    let namorOpts = { words: numStrings, saltLength, saltType, separator }
    if(isManly) {
        namorOpts = {...namorOpts, subset: `${MANLY}` }
    
    return isManly
                ? namor.generate({ words: numStrings, saltLength, saltType, separator, subset: `${MANLY}` })
                : namor.generate({ words: numStrings, saltLength, saltType, separator })
}
