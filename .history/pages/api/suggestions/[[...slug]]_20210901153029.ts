...
export default getHandler()
    .get('/api/suggestions/random', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            let suggestion1 = await suggestRandomUniqueSlug(saltLength, numStrings, isManly)
            let suggestion2 = await suggestRandomUniqueSlug(saltLength, numStrings, isManly)
            
            res.status(200).json({ suggestion: !suggestion1 ? suggestion2 : suggestion1 })
        } catch(error) {
...
async function suggestRandomUniqueSlug(saltLength: number, numStrings: number, isManly: boolean) {
    ...
    await redis.hget(`slug.${suggestedRandomSlug}`).then((response) => {
        return response===null ? suggestRandomSlug : null;
    }).catch((error) => {
