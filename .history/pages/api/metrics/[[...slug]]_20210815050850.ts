...
export default getHandler()
    .get('/api/metrics/clickstream/recent/:time/:unit',  async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(time && unit && (unit==='hours' || unit==='mins' || unit==='secs') && multiplier!==-1) {
            ...
            const iterator = freqs.entries(); 

            let logger = []; 
            let cursor = iterator.next().value
            while( cursor !== undefined) {
                logger.push(`${cursor}::::->`)
                let freqsIterator = cursor.entries();
                let freqsCursor = freqsIterator.next().value;

                if(freqsCursor !== undefined) {
                    logger.push(`--${freqsCursor}--`)
                } else {
                    break; 
                }
            }
            ...
            res.status(200).json({ timeseries, clickstream, logger, start, end });
        } else {
