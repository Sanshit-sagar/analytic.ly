...
export default getHandler()
    ...
    .get('/api/metrics/slug/:slug/views', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        http://localhost:3000/api/responses/user/sanshit.sagar@gmail.com/times
        if(slug) {
            const data: { id: string; cfRay: string; timestamp: number; user: string; slug: string; }[] = await getDoubleEndedClickstream(0, -1, slug, 'slug', true);
