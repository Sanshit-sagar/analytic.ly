...
function getGroupedData(detailedClicks: DetailedClick[]) {
    return Array.from(group(detailedClicks, (dclick: DetailedClick) => dclick.timestamp))
}
...
export default getHandler()
    ...
    .get('/api/metrics/user/:email/clickstream/grouped', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        try {
            const clicks = await getClickstream(email)
            const groups = groupBySeo(clicks)
