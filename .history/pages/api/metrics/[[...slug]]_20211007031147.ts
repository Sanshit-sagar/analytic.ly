...
function getGroupedData(detailedClick: DetailedClick) {
...
export default getHandler()
    ...
    .get('/api/metrics/user/:email/clickstream/grouped/:criteria', async (req: NextApiRequestExtended, res: NextApiResponse) => {
        const email = req.params.email

        if(email) {
            try {
                const clicks = await getClickstream
            }
        }
    })