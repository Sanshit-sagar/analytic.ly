...
import { getDoubleEndedClickstream, IAggregatedClicks } from '../../../lib/redis/clicks'
...
export default getHandler()
    .get('/api/metrics/clickstream/recent/:time/:unit',  async (req: NextApiRequestExtended, res: NextApiResponse) => {
        ...
        if(time && unit && (unit==='hours' || unit==='mins' || unit==='secs') && multiplier!==-1) {
            ...
            const {aggregatedClicks, phsArr}: IAggregatedClicks = await aggregateClicksForTimeseries(timeseries, clickstream); 

            res.status(200).json({ timeseries, clickstream, aggregatedClicks, start, end, phsArr });
        } else {
