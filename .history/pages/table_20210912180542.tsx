...
const TabulatedClicks = () => {
    ...
    const clicks: any[] = data ? [].concat(...data.page) : [];
    const isLoadingInitialData = !data && !error
    const isLoadingMore = isLoadingInitialData || (size > 0 && data && typeof data.page[size-1]===UNDEFINED)
    const isEmpty = data?.page?.[0]?.length === 0 
    const isReachingEnd = isEmpty || (data && (data.page[data.page.length -1] < PAGE_SIZE))
